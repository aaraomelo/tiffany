...
operador deflacionário $T\preceq\mathrm{id}$ para a delegação --- do que os
teoremas (não-escalada, propagação limitada, fail-closed) seguem como
corolários, e a profundidade $\dep$ aparece como iteração de $T$ rumo ao seu
maior ponto fixo. O Apêndice~\ref{ap:plano} traz o plano de implementação.
\end{abstract}
...
\begin{itemize}[nosep,leftmargin=1.3em]
  \item \textbf{Profundidade $=$ iteração de $T$.} Re-delegar $k$ vezes é
        $T^k$. Como $T\preceq\mathrm{id}$ e monótono, a cadeia
        $x\succeq T(x)\succeq T^2(x)\succeq\cdots$ é decrescente. A
        intensidade $\dep=k$ é o número de iterados; $\dep=\infty$ é o
        \textbf{maior ponto fixo} $\nu T=\bigsqcap_{n}T^n(x)$ (Kleene de
        cima, garantido por Knaster--Tarski no reticulado completo).
        ``Propagação infinita'' $=$ ponto fixo; ``$\dep$ finito'' $=$
        iterado $T^{\dep}$.
  \item \textbf{Não-escalada $=$ monotonicidade.} $p\mapsto E_p$ é um
        morfismo monótono do DAG de delegação (ordenado por derivação) para
...
auditáveis e a cascata de revogação sobre o DAG. Os dois compõem sem se
contaminar.
\end{remark}
...
muda. Revogar = \texttt{UPDATE state='revoked'} em $I$ e em toda a sua
descendência por \texttt{parentId} (CTE recursiva). Atenuação de $\tau$ no
\emph{grant-check} (espelho de \texttt{attenuatedDepth}). Transições
auditadas; \emph{guard} = autoridade sobre a instância.
