...
$u=2^{-\operatorname{prof}}$ & a \textbf{ultramétrica estrutural} de endereço & \code{fis:def:arvore}; \code{fis:thm:troca-realizacao} \\
...
$d(a,b)=u(a,b)=2^{-\operatorname{prof}(a,b)}$ da árvore do encaixe
(\code{fis:def:arvore}). \emph{A âncora da memória é a bola $\mathcal{B}(a,r)$
dessa árvore.} A invariância $u(\partial a,\partial b)=u(a,b)$ sob a troca dual
$R,R^{\dagger}$ está no \fis{fis:thm:troca-realizacao}{Teor. «troca de
realização»} --- \emph{não} se afirma aqui para leituras arbitrárias de $\pi$.
...
baratos: o livro já os afirma quase nas mesmas palavras. \pt{6} traduz o outro
como superposição já determinada pela física (\code{fis:thm:combina}); Hebb é
o \code{fis:cor:hebb}, não decreto. Aqui lê-se na §\ref{mf:outro}.
...
% ───────────────────────────────────────────────────────────────────────────
\section{Régua: troca de realização}\label{mf:troca}

\noindent O \fis{fis:thm:troca-realizacao}{Teor. «troca de realização»} do
\code{fisica.tex} é a régua que este documento \emph{já usava} sem
reconstruí-la: foco como realização $R_F$, e --- \emph{neste domínio} da lista
binária e das leituras do \code{fis:thm:B} --- o mapa
$D_{\mathrm{can}}=R^{\dagger}\circ R^{-1}$ com $D_{\mathrm{can}}(i)=(N-1)-i$,
donde $\operatorname{prof}$ e $u=2^{-\operatorname{prof}}$ se preservam
(\code{fis:thm:dual}(5)). \emph{Não se generaliza}: leituras distintas de $\pi$,
travessias espectro/célula e soma Hebb são outros eixos.
\emph{Não se identifica} $D_{\mathrm{can}}$ com $\tau$
(\code{fis:obs:cisao}): $\tau$ parte o par; $D_{\mathrm{can}}$ complementa o índice.
Quem determina $R^{\dagger}$ é o \code{fis:thm:B} (segunda nomeação,
$0\leftrightarrow 1$), não $\partial^{2}=\mathrm{id}$. O relógio lê
$u=2^{-\operatorname{prof}}$; não escolhe o operador, e não se acrescenta
frequência sobre a árvore.
O par $(A_{\mathrm{prof}},A_d)$ do relógio (\code{fis:obs:relogio-dual}) é a
leitura ultramétrica desse $D_{\mathrm{can}}$, \textbf{não} uma lei de combinação
de conteúdos.
...
\item[\textbf{(II)}] \textbf{Instância com hipóteses já presentes} --- formalizar
como corolário local (ex.: \pt{4} + bolas).
\item[\textbf{(III)}] \textbf{Exige mais} --- marcar a hipótese extra; não
inventar $u(Da,Db)=u(a,b)$.
\end{enumerate}

\medskip\noindent\textbf{Mapa dos resultados deste documento.}
...
\toprule
resultado & est. & relação com \code{fis:thm:troca-realizacao} \\
\midrule
\pt{4} & I & $u=2^{-\operatorname{prof}}$: ultramétrica \textbf{estrutural}
(\code{fis:def:arvore}) \\
\code{fis:thm:bolas} (bolas/partição) &
I/II & citar o teorema neste domínio; isometria sob $D_{\mathrm{can}}$ se
$R,R^{\dagger}$ canónicas \\
Teor.~\ref{mf:thm:ancora} & II/III &
foco = realização local (bola); isometria só se leituras $R,R^{\dagger}$
identificáveis (Cor.~\ref{mf:cor:troca-u}) \\
Teor.~\ref{mf:thm:reconstroi} & III &
$\sum G=\abs I$ conserva-se; $\pi\neq\pi'$ pode coincidir na medida \\
Cor.~\ref{mf:cor:ponte} & III &
travessia célula/espectro (\code{fis:thm:ponte}); não $D_{\mathrm{can}}$ \\
Teor.~\ref{mf:thm:outro} (Hebb) & --- &
descida de \code{fis:thm:combina}/\code{fis:cor:hebb}; \emph{não} segue da troca \\
foco clínico (Rach.~V) & II/III &
território = bola; dual canónico exige $R,R^{\dagger}$ explícitas \\
Teor.~\ref{mf:thm:duas} & III &
eixo \code{fis:thm:duastorres}; distinto de \code{fis:def:duomorf} \\
\bottomrule
\end{tabular}}
\end{center}

\begin{corolario}[Isometria da troca dual no multifocal]\label{mf:cor:troca-u}
Sob \pt{4}, com $u(a,b)=2^{-\operatorname{prof}(a,b)}$ e leituras reversíveis
$R,R^{\dagger}$ no sentido do \fis{fis:thm:troca-realizacao}{Teor. «troca de
realização»},
\[
u(Da,Db)=u(a,b),
\qquad D=R^{\dagger}\circ R^{-1}.
\]
\emph{Mudar o raio ou o prefixo altera a \textbf{realização} do território; $u$
permanece estrutural sob a troca dual demonstrada.}
\end{corolario}

\begin{proof}
É o \fis{fis:thm:troca-realizacao}{Teor. «troca de realização»}, cláusula~(4),
com $u=d$ da \code{fis:def:arvore} importada por \pt{4}.
\end{proof}

\medskip\noindent\textbf{Três níveis --- e o que \pt{4} não autoriza.}
A cisão $\tau\to(X_{+},X_{-})$ é \code{fis:obs:cisao}. A dualidade do relógio
\[
A_{\mathrm{prof}}=\min\circ\operatorname{prof}
\;,\qquad
A_d=\max\circ d
\]
é \code{fis:obs:relogio-dual}: profundidade pelo mínimo, distância pelo máximo,
dobra $D_{\mathrm{can}}(i)=(N-1)-i$. A isometria $u(Da,Db)=u(a,b)$ é do
\textbf{endereço}. Não implica
$D_{\mathrm{can}}(f(x,y))=f(D_{\mathrm{can}}x,D_{\mathrm{can}}y)$ sobre o
conteúdo da representação. Donde os invariantes de endereço \textbf{não}
autorizam $X\times X\to X$. Quem determina a combinação é o
Teor.~\code{fis:thm:combina}. \pt{6} realiza essa lei na leitura do outro; não
a legisla. $G>1$ é consequência da soma, não axioma.
\[
\boxed{\;
C\;\longrightarrow\;S\;\longrightarrow\;\Delta_{\mathrm{int}}
\;\longrightarrow\;W_{ij}.
\;}
\]
$\tau$, $D_{\mathrm{can}}$, $\operatorname{lexMax}$ e o pipe ficam \emph{fora}
desta derivação. O relógio lê $u$; não escolhe $R^{\dagger}$.
\code{papers/redes.tex} só realiza.
...
\end{enumerate}
\emph{Classificação pela régua (\S\ref{mf:troca}):} cláusula~(1) conserva contagem;
cláusulas~(2)--(3) mostram que \emph{realizações} distintas podem coincidir na
medida --- estatuto \textbf{III}; não se conclui $u(Da,Db)=u(a,b)$ entre focos
arbitrários.
\end{teorema}
...
propriedades que o livro precisava de postular passam a ser cláusulas. \emph{O
foco} clínico é realização local (bola); a isometria $u(Da,Db)=u(a,b)$ entre
focos duais canónicos está no Cor.~\ref{mf:cor:troca-u}, não em qualquer par de
territórios.
...
\emph{A isometria $u(Da,Db)=u(a,b)$ sob troca dual canónica não se reprova aqui:
está no Cor.~\ref{mf:cor:troca-u} via \code{fis:thm:troca-realizacao}.}
...
\emph{Estatuto \textbf{III} (\S\ref{mf:troca}):} é \code{fis:thm:ponte}, não a troca
$D=R^{\dagger}R^{-1}$ do \code{fis:thm:troca-realizacao}.
\end{corolario}
...
\noindent Aqui \pt{6} realiza a combinação física (\code{fis:thm:combina}) na
leitura do outro. \code{papers/redes.tex} só realiza; não legisla. Hebb é o
\code{fis:cor:hebb}. $G>1$ é consequência da soma, não axioma. É o mecanismo
que o livro não tem para a palavra \cury{inevitável}.

\begin{teorema}[somar identifica; separar custa espaço]\label{mf:thm:outro}
Sob \pt{6} --- que realiza o \code{fis:thm:combina} na leitura do outro ---,
com $W_{ij}=\sum_{p}\xi^p_i\xi^p_j$ o \code{fis:cor:hebb}:
\begin{enumerate}\itemsep3pt
...
chama-se \textbf{interferência}. $G>1$ \emph{não} é posto à mão: aparece porque
$\sum_p\xi^p$ não é injetiva.
...
com $W$ o \code{fis:cor:hebb} da superposição \code{fis:thm:combina}.
...
\pt{6} & \code{fis:thm:combina}, \code{fis:cor:hebb}, \code{fis:thm:interfere} \\
...
\medskip\noindent\textbf{O compromisso.} \pt{6} como realização da combinação
física, e com ele uma consequência
...
a desescreve --- com $u=2^{-\operatorname{prof}}$ do lado estrutural
(\S\ref{mf:troca}, \code{fis:thm:troca-realizacao}). \emph{Não é que a construção sirva para tudo}; é que o livro
