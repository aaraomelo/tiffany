...
\smallskip
\noindent E o mecanismo é agora \textbf{propriedade operacional, medida}: a luz da dimensão é a
meia volta $c_n=q_n/2$ do seu relógio de $q_n=n\,2^{n-1}$ marcas ($32=2^5$ em $n=4$), a lei da
torre escreve-se \emph{sem} $\pi$ --- cada passo $n\to2n$ multiplica o relógio por $2^{n+1}$, e o
$\pi_n$ da meia-corda de Arquimedes era o transporte, não a lei ---, e \textbf{o limite dimensional
é função de $c_n$}: cada $c$ determina o seu $n$, e o percurso fecha exactamente quando $n$ é par.
E a relação $c_n=\pi_n R_n$ \textbf{não é identidade por construção}: o teste decisivo corre em dois
instrumentos exactos --- a álgebra ($G^{q/2}=-I$, a meia volta) e a geometria (no passo $q/2$ a
posição é $(1,1)$, $D^{2}=2$) ---, com a órbita a fechar na origem sem uma raiz
(\code{tests/luz\_periodo.c}, §L6a--§L6b).
