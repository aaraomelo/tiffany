...
% corpus/docs/medida.tex — a teoria da medida deste sistema, derivada do corpo estelar.
...
\begin{document}
\gkcapa{A Medida}
       {saltar sobre o irracional, furar o que não conta}
       {contar, saltar, furar --- e nenhum dos três é aproximar}
\maketitle
...
f^{-1}$, a Lei~1 (a involução). A função que vale $1$ nos racionais
...
colapsados). Esta partição em \emph{duas} famílias é a vista grosseira: refina-se na progressão de seis
(\emph{Catálogo}, \code{obs:seis-leis}), onde Riemann, BSD e Hodge são os três eixos do \textbf{trial}
$\{-1,0,+1\}$ e Navier--Stokes é a \textbf{tetral} --- e as seis leis são as \emph{primitivas
operacionais} que fundamentam também este paper.

\smallskip
\noindent \textbf{E a vista fina fecha em oito, não em seis} (\emph{corpo estelar}, a subsecção
\code{sub:oitoleis}): a progressão renumera-se de $0$ a $7$, uma lei por degrau, e as \emph{duas leis}
deste teorema são a \textbf{Lei~1} (o dual --- a Poincaré-dualidade, Riemann) e a \textbf{Lei~2} (o
bidual $K^{**}=K$, cujo ponto fixo é o \textbf{centro} $0$, onde ficam o Yang--Mills e o $P$ vs $NP$
--- aqui apenas \emph{localizados} na torre, não resolvidos). E as pontas são os dois nulos que estas
duas pressupõem: a \textbf{Lei~0} --- a divisão do zero, $0=(+1)\oplus(-1)$ com $0\dg=\infty$ --- é a
base do $0$ e do $\infty$, e o seu limite é o ponto fixo onde \emph{contar}$=$\emph{integrar}, o
Gentil--Lebesgue--Hurwitz que o teorema central (\S\ref{sec:central}) funda. A torre inteira desce
$7\to0$, e não há nona.
...
inversa, a Lei~1 (a involução). A conservação que já está medida nos metais,
\[
...
\subsection*{O teorema operacional da medição: medir pela metade refletida}\label{sec:operacional}

\noindent A proibição acima --- \emph{não pertence, por não haver como voltar} --- não é uma regra
sobre conjuntos: é uma regra sobre \textbf{como se mede}. Ela deriva-se, e dá a razão de todas as
outras proibições deste sistema.

\begin{teorema}[a medição fiável é a reversível, e o Lyapunov sai da metade refletida]\label{thm:operacional}
Uma medição é um transforma $T$; a sua \textbf{metade refletida} é a inversa $T^{-1}$ (a inversa
temporal). Então:
\begin{enumerate}
\item \textbf{o Lyapunov deriva-se da metade refletida}: num sistema reversível o espectro de
expoentes é \textbf{simétrico}, $\lambda^+ + \lambda^- = 0$ (a inversa temporal nega o expoente),
logo $\lambda=(\lambda^+-\lambda^-)/2$ mede-se por \emph{uma} metade e reflecte-se a outra;
\item \textbf{a medição é fiável se e só se é reversível}: $T^{-1}(T(x))=x$ para todo $x$ com resíduo
zero. Uma medição que \emph{perde} um bit não tem inversa e o resíduo da volta é $>0$: \textbf{não é
fiável} --- não volta. Isto \emph{não} a torna, por si só, caótica (um passo não-injectivo pode até
contrair); o caos aparece quando esse resíduo entra num \emph{estado que o acumula}: aí cada passo
dobra-o e $\lambda_{\text{med}}>0$ (\code{tests/estado\_caos.c}~§D1, o estado é o canal do caos). Sem
estado que acumule, o erro fica \emph{limitado} (\code{tests/lyapunov\_refletido.c}~§L4, $(2^d-1)\sum x$),
mas a medida continua sem volta.
\end{enumerate}
No \emph{Corpo de Peano} esta medição realiza-se como \textbf{Teorema do Metrónomo}:
o maestro projecta $\pi_k$; o metrónomo lê o relógio; o Lyapunov dualizado atesta
$\lambda^{+}{+}\lambda^{-}=0$ na classe operacional
(\code{papers/corpo\_topologico.tex}, Teorema do Metrónomo;
\code{tests/corpo\_topologico.c}~§CP40).
\end{teorema}

\begin{proof}
Reversível $\Rightarrow$ a cada $\lambda$ da frente corresponde $-\lambda$ da inversa (a mesma
trajectória lida ao contrário afasta-se à taxa oposta): o espectro emparelha $\pm\lambda$ e soma zero.
E $T^{-1}\circ T=\mathrm{id}$ com resíduo $0$ diz que nada se perde entre uma leitura e a seguinte
(\code{tests/lyapunov\_refletido.c}~§L3: a reversível devolve $0$, a com perda não). Um passo com perda
--- um bit descartado --- não é injectivo: não há $T^{-1}$ que o desfaça, o resíduo da volta é $>0$, e
isso basta para \textbf{não ser fiável}. Se esse resíduo entra na recorrência de um \emph{estado que
acumula}, cada passo dobra-o e $\lambda_{\text{med}}>0$ (\code{tests/estado\_caos.c}~§D1); num passo
sem estado, o erro é limitado, mas a volta falha na mesma. A volta é a medida; sem ela não há medida,
há deriva.
\end{proof}

\noindent \textbf{E é esta a justificativa das proibições.} O \code{double} \emph{arredonda} ---
descarta um bit em cada operação, e não há volta; e num documento grande, que \emph{acumula}, propaga
com $\lambda_{\text{med}}>0$ (\code{tests/assinatura\_tradutor.c}~§A3). O \code{malloc} e
o \emph{estado} guardam para reescrever --- a escrita apaga o que estava, e o apagado não volta. O
\emph{buffer} é a cópia que introduz o atraso que oscila. Todos partilham o mesmo defeito, e é um só:
\textbf{perdem a metade refletida}. Por isso a régua deste sistema é o inteiro (não arredonda), o
disco reversível (não apaga), e o medidor que \emph{compara dois caminhos} --- a frente e a volta ---
em vez de asseverar um número. Medir é ir e voltar; o que não volta não se mede: \textbf{não pertence}.

\smallskip
\noindent \textbf{E o erro quantifica-se em atrasos unitários.} O atraso é o $z^{-1}$: um passo a mais
de um lado. Balanceada --- expande $N$, contrai $N$ --- a metade refletida cancela e o resíduo é $0$
(factor de potência um). Aplicando $d$ atrasos unitários \emph{num} lado, a reflexão fica $d$ passos
atrás e sobra o factor $e^{\lambda d}$: o erro é $(e^{\lambda d}-1)\sum x$, e \textbf{cada atraso
unitário multiplica o erro por $e^\lambda$}. O desbalanço é a \emph{reactância} --- o atraso que não
cancela ---, e cresce com o Lyapunov: um só lado com estado basta para a medição derivar. O remédio
não é um segundo \emph{buffer}: é não ter o primeiro, e deixar a frente e a volta fecharem sem atraso.

\medido{o Lyapunov deriva-se da metade refletida: iterando o dobrador $s\mapsto2s$ a separação mede
$\delta^+(n)=2^n$ (expoente $e^+=n$, $\lambda^+=1$ bit/passo $=\log2$), e a metade refletida (a inversa
$s\mapsto s/2$) desfá-la exactamente --- $e^++e^-=0$, volta com resíduo $0$ em $19\,999$ valores; e o
teorema operacional: a estrela reversível $\nu\circ\nu=\mathrm{id}$ volta (resíduo $0$, fiável),
enquanto a medida com perda $g(x)=\lfloor x/2\rfloor\cdot2$ (um bit descartado, como o \code{double} ou
o estado sobrescrito) não volta ($10\,000$ de $19\,999$ com resíduo $>0$); e o erro quantifica-se em
atrasos unitários --- $d$ atrasos ($z^{-1}$) num lado dão erro $(2^d-1)\sum x$, cada atraso a multiplicar
por $2=e^\lambda$, e $d=0$ (balanceado) a fechar em resíduo $0$: \code{tests/lyapunov\_refletido.c}.}

\subsection*{A assinatura é a metade refletida: a config é um corpo, e confere-se}\label{sec:assinatura}

\noindent E é assim que uma medição \emph{atravessa uma fronteira}. Quando o parser (nativo) entrega a
\textbf{config} ao núcleo (que sobe a \emph{wasm}), a config não vai crua: vai como um \textbf{corpo com
assinatura}. E a assinatura não se inventa --- recupera-se do corpo técnico, onde os formatos de ficheiro
já a têm: um formato de \emph{texto} é um corpo $(\text{marca},\text{razão},\text{sinal})$ (a cifra do seu
gerador), e um registo \emph{binário} assina-se $\{\textsc{magic},\text{tamanho},\text{crc}\}$, com o leitor
a \textbf{conferir}. A config é binária: assina-se como o registo, e entra no mesmo catálogo dos formatos.

\smallskip
\noindent \textbf{E o conferir é a metade refletida.} O \textsc{crc} \emph{guardado} é a assinatura; o
\textsc{crc} \emph{conferido} é a volta; e a medição só é fiável se os dois batem --- exactamente o
Teorema~\ref{thm:operacional}. Um bit vazado no atravessamento muda o conferido e o leitor recusa; não se
\emph{chuta} que a config chegou intacta, \emph{confere-se} contra a sua própria assinatura. É a Lei~1 na
fronteira: o corpo entra, atravessa, e a régua que o assina é a que o verifica do outro lado.

\medido{a config dos parsers é um corpo assinado, e a assinatura recupera-se do corpo técnico (não se
inventa): as cifras dos formatos de texto de \code{lib/caminho.h} são distintas (cada formato é um corpo
$(\text{marca},\text{razão},\text{sinal})$), e o binário assina-se $\{\textsc{magic},\text{len},\textsc{crc32}\}$
de \code{lib/banco.h} --- o \textsc{crc} é o IEEE real (bate com o padrão $\texttt{crc32(\textnormal{``}123456789\textnormal{''})}=\texttt{0xCBF43926}$),
o leitor confere os três e o round-trip devolve a config byte-a-byte; um bit trocado muda o \textsc{crc}
conferido e o leitor recusa (a metade refletida, resíduo $0$ ou recusa): \code{tests/config\_corpo.c}.}
...
\begin{proof}
A bijeção dual é a involução $f\leftrightarrow f^{-1}$, a Lei~1, já medida acima
...
outro}), e é aí que a norma bilinear fecha e a seguir quebra. O lado contínuo mede pela norma, um
