...
\part{O corpo dual e as duas leis}
...
correspondência \emph{continuamente} (Prop.~as áreas no \emph{Catálogo}). O que a dimensão nova traz é uma
...
quadráticos} --- as palavras periódicas, por Lagrange (Prop.~a bijeção de Lagrange no \emph{Catálogo}). Não é uma
...
medida e executável}: Prop.~o degrau $\Z\times\N^{*}\to\R$ no \emph{Catálogo}, \code{tests/palavra.c} ($33$ unidades, resíduo $0$)
...
\hline
Pontryagin & o eixo de Pontryagin no \emph{Catálogo} --- e é \emph{o eixo} das três partes & $G\cong\widehat{\widehat G}$; compacto\,$\leftrightarrow$\,discreto, com $\widehat\R=\R$ no ponto fixo \\
...
$\Z\times\N^{*} \to \R$ & \textbf{bijeção} & \textbf{sim} --- Möbius, Prop.~o degrau $\Z\times\N^{*}\to\R$ no \emph{Catálogo} \\
...
construção de $\R$ (a construção de $\R$ no \emph{Catálogo}) cabe nesta escada sem emenda.
...
$\Z\times\N^{*} \leftrightarrow \R$ & a mesma Möbius, sem parar & \textbf{bijeção} (Prop.~o degrau $\Z\times\N^{*}\to\R$ no \emph{Catálogo}) \\
\hline
\end{tabular}
\end{center}
...
a mesma nos dois andares. Está enunciado na Prop.~o degrau $\Z\times\N^{*}\to\R$ no \emph{Catálogo} e medido no \code{palavra.c}.
...
$\R^n \to \Z$ & o traço $t_k=\sigma^k+\sigma'^k$ & \textbf{inteiro do corpo} & no \emph{Catálogo} \\
...
nova (Prop.~o degrau $\Z\times\N^{*}\to\R$ no \emph{Catálogo}); para $\zeta\to\sigma$ é a Prop.~\ref{prop:betazeta}.
\end{proof}
...
simétrica e a antissimétrica (as duas metades: traço e discriminante no \emph{Catálogo}) --- o par dual, outra vez.
...
palavra\,$\to$\,matriz já dá por construção (o dicionário palavra$\to$matriz no \emph{Catálogo}): \emph{a condição do dual em $\Z$ e a
...
(onde Pisot falha no \emph{Catálogo}), e em $n=5$ os dois enunciados divergem. \textbf{É a dimensão que decide},
