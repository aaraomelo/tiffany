...
\section*{Sobre o que aqui se reivindica, e sobre o que se credita}
\addcontentsline{toc}{section}{Sobre o que se reivindica e o que se credita}

\noindent Este volume tem uma política editorial e ela cabe em duas frases.

\medskip
\noindent \textbf{A teoria é reivindicada por inteiro.} O corpo dual $(V,V^{*},\dagger)$ como
ambiente, a estaca e a cruz como as duas notações primitivas, as duas leis --- \emph{a unidade é
dual} e \emph{a dualidade é dual} ---, a torre como iteração da cruz, o tempo como assinatura da
régua e $\pi$ como o meio-período que realiza a Lei~$1$: tudo isto é enunciado, provado e
sustentado aqui, e é o que este volume reclama como seu. Não se pede licença a nenhuma tradição
para o dizer, e nenhum destes enunciados é apresentado como leitura de outro.

\medskip
\noindent \textbf{O crédito está todo no bestiário.} As realizações --- os corpos com nome, as
estruturas clássicas, os teoremas que este texto \emph{usa} sem os ter descoberto --- vivem no
\emph{Catálogo}, e é lá que cada um aparece com o seu autor e a sua data. Isso não é modéstia
distribuída em rodapés: é a mesma separação que organiza os três volumes. \emph{Quem constrói
fica aqui; quem já estava fica lá, com o nome escrito por extenso.}

\medskip
\noindent \textbf{E o critério de fronteira é a derivação, não a precedência.} Um enunciado que
se \emph{deriva} aqui --- do corpo dual e das duas leis, sem invocar nada de fora --- é um
teorema deste texto, e é apresentado como tal. Que alguém tenha chegado antes ao mesmo enunciado
por outro caminho não muda o estatuto dele nesta construção: muda o \emph{sítio} onde essa
precedência fica registada, que é o bestiário, com autor e data.

\medskip
\noindent Isto não é uma reivindicação sobre prioridade histórica --- é uma afirmação sobre
\emph{arquitetura}. Dizer que um resultado é teorema deste texto é dizer que ele \textbf{sai das
duas leis}, e portanto que a teoria o alcança sem empréstimo. \emph{Onde a derivação existe, o
resultado é daqui; onde ela não existe, o resultado não é dito aqui de todo} --- é dito no
bestiário, no nome de quem o provou. A fronteira é essa, e é verificável linha a linha: basta
perguntar, em cada enunciado, se ele se segue do que já foi estabelecido.
