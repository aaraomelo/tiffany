...
coeficiente --- é a adjunção (\S\ref{thm:lei3}) a fazer o trabalho, e nada mais é usado. A
...
com um pai só, que é o teorema da dimensão como contagem (\S\ref{thm:dimconta}) lido como figura.
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{a unidade é\dots} & \textbf{o que se pede} & \textbf{o que sai} & \textbf{a parte} \\
\midrule
inteira    & que se conte           & degraus, vizinhança, o discreto & \textbf{Topologia} \\
racional   & que se opere e inverta & o corpo, e as suas operações    & \textbf{Álgebra} \\
real       & que se aperte até ao limite & o encaixe, a taxa, o tempo & \textbf{Análise} \\
completa   & que se feche a volta   & a fase, a curvatura, a figura   & \textbf{Geometria} \\
\bottomrule
\end{tabular}
\end{center}
...
\noindent E os dois pares são um par, como já se disse: o primeiro é a \emph{estrutura} --- o que
existe e como se combina; o segundo é a \emph{dinâmica} --- o que corre e que figura deixa.
...
\begin{proof}
Escrevendo a leitura em coordenadas, $\langle u,v\rangle=\sum_i u_i v_i$, tem-se
$\langle Ax,y\rangle=\sum_i\bigl(\sum_j A_{ij}x_j\bigr)y_i=\sum_{i,j}A_{ij}x_jy_i$. Trocar a ordem
da soma dá $\sum_j x_j\bigl(\sum_i A_{ij}y_i\bigr)=\langle x,A^{\top}y\rangle$. \emph{O único
passo é trocar a ordem de uma soma finita} --- e é por isso que a identidade não pede hipótese
nenhuma sobre $A$: ela é a comutatividade da escrita, lida duas vezes. $\square$
\end{proof}
...
\section{A divisão deste texto é ela própria dual}\label{sec:divisaodual}

\noindent Uma teoria que afirma que tudo vem em pares deve poder ser lida na sua própria
organização, ou a afirmação não se aplica a si mesma. Aplica-se, e vale a pena dizer como, porque
a estrutura das quatro partes \emph{não} é uma sequência de quatro assuntos: são \textbf{dois
pares, e os dois pares formam um par}.

\begin{center}\small
\begin{tabular}{@{}llll@{}}
\toprule
& \textbf{um lado} & \textbf{o outro} & \textbf{o que a estaca troca} \\
\midrule
\textbf{estrutura} & Álgebra & Topologia & discreto $\leftrightarrow$ compacto \\
\textbf{dinâmica}  & Análise & Geometria & tempo $\leftrightarrow$ espaço \\
\bottomrule
\end{tabular}
\end{center}

\noindent \emph{O primeiro par é o eixo que atravessa este texto inteiro:} a álgebra opera e não
alcança a completude; a topologia alcança a completude e não opera. Nenhuma das duas fecha
sozinha, e a passagem de uma à outra troca discreto por compacto --- que é a forma que a estaca
toma neste nível.

\medskip
\noindent \emph{O segundo par é o mesmo, uma dimensão acima:} a análise dá o parâmetro que corre,
a geometria dá a figura que ele deixa. E a ponte entre eles já foi construída --- o contador leva
o parâmetro contínuo ao inteiro, e o operador que curva o espaço é o mesmo que gera o tempo.

\medskip
\noindent \textbf{E os dois pares são um par}: estrutura e dinâmica, o que não se move e o que
move. É a bidualidade lida na própria divisão --- \emph{o texto tem a forma daquilo que
descreve}, e isso não é ornamento: é a única disposição em que cada parte tem onde ir buscar o
que lhe falta.




\begin{teorema}[a dimensão conta as involuções]\label{thm:dimconta}
Num objeto obtido por iteração da cruz, a dimensão não é um dado independente: $\dim A_n =
2^{\,n}\dim A_0$, e portanto $n=\log_2(\dim A_n/\dim A_0)$. \textbf{A dimensão é o número de
vezes que se dualizou}, lido noutra escala.
\end{teorema}

\begin{proof}
Imediato do Teorema~\ref{thm:torrecruz}: cada aplicação da cruz junta a um objeto o seu dual, que
tem a mesma dimensão, logo duplica. Inverter $2^{n}$ dá $n$. \emph{E a recíproca é o conteúdo}:
como $n$ se recupera da dimensão, dois andares de dimensão igual são o mesmo andar --- não há
como chegar à mesma dimensão por dois números diferentes de dualizações. $\square$
\end{proof}

\noindent \emph{Daí a dimensão ser uma contagem e não uma grandeza.} Ela não mede quanto há; mede
quantas vezes se trocou de lado. É por isso que subir de dimensão não acrescenta tamanho ---
acrescenta uma fase, que é a marca de mais uma involução.

\begin{teorema}[há exatamente duas involuções, e elas não se confundem]\label{thm:duasinvol}
Sobre um corpo dual com as duas operações existem duas involuções estruturalmente distintas:
\[
  \dagger:\ \text{preserva } \oplus,\ \text{inverte a ordem de } \otimes;
  \qquad
  \mathcal{D}:\ \text{leva } \oplus \text{ a } \otimes .
\]
Em geral $\dagger\neq\mathcal{D}$, e nenhuma identidade de uma pode ser transportada para a outra
sem demonstração.
\end{teorema}

\begin{proof}
A estaca preserva a escrita --- $(x\oplus y)^{\dagger}=x^{\dagger}\oplus y^{\dagger}$ --- e
inverte a ordem da leitura: $(ST)^{\dagger}=T^{\dagger}S^{\dagger}$. Já $\mathcal{D}$ transporta
uma operação na outra, $\mathcal{D}(x\oplus y)=\mathcal{D}(x)\otimes\mathcal{D}(y)$, o que a
estaca não faz. Se as duas coincidissem, a estaca satisfaria as duas identidades ao mesmo tempo,
e então $\oplus$ e $\otimes$ seriam a mesma operação --- contra a hipótese de haver duas.
$\square$
\end{proof}

\medido{A identidade $\mathcal{D}$ testada sobre a estaca falha em $21$ de $6561$ casos --- não
falha sempre, e é por isso que confundi-las é fácil: coincidem numa maioria e separam-se numa
minoria que basta para as distinguir.}

\noindent \textbf{É esta segunda involução que troca o aditivo pelo multiplicativo}, e é ela ---
não a estaca --- que responde por toda a família de correspondências entre uma escala e a sua
logarítmica. Onde uma transformada leva soma em produto, é $\mathcal{D}$ que está a agir; onde
uma troca lados preservando a soma, é $\dagger$. \emph{As realizações dessas duas famílias estão
no \emph{Catálogo}, e cada uma declara qual das involuções usa.}
...
(\S\ref{thm:duasinvol}) --- a que leva o produto à soma. As duas quantidades ficam \emph{aditivas}
e o par mostra-se:
\[
