...
- **[A asserção que passa sem poder falhar](feedback-assercoes-vazias.md)** — OITO formas (constante disfarçada, tabela literária, número de cabeça, caso degenerado, limiar no valor exato…). Antes de commitar: que entrada faria esta asserção falhar?
...
- [Normalizar não é medir](feedback-normalizar-nao-e-medir.md) — dividi uma quantidade por si própria e chamei-lhe confirmação. Uma tabela inteira de 1 é mais vezes tautologia que lei.
...
- [O que justifica a involução](feedback-justificar-o-que-so-e-coerente.md) — a conservação obriga, mas só onde é ADITIVA; nos corpos algébricos é multiplicativa (|N|=1). Erro repetido: números certos, falsa a frase que os ligava.
- **[O número que não cabe](feedback-o-numero-que-nao-cabe.md)** — o teste mais barato contra um número escrito à mão: **cabe no tipo?** A «máquina de 80 bits» era um `uint64_t`.
...
- **[O medidor que nunca mediu](feedback-o-medidor-que-nunca-mediu.md)** — QUATRO diziam «NÃO MEDIU» e a bateria contava-os VERDES: a atestação guarda o *resultado*, não o *motivo*. Apanhados por a ASSINATURA MUDAR, não por asserção. Procurar `exit != 0` custa um `awk`.
- **[O \medido sem medidor](feedback-o-medido-sem-medidor.md)** — 28 blocos afirmam número e resíduo sem nomear programa: a bateria só conta o CITADO. Cruzar por números não os recupera.
...
- **[A estrutura lida como ruído](feedback-estrutura-lida-como-ruido.md)** — um resíduo que não fecha pode ser **meia órbita**, não erro de medida. Perguntar **o período do operador** ANTES de medir dispersão. Sinal: a «volta» dá o mesmo que o controlo.
...
- **[A dualidade é a memória da divisão](project-dualidade-memoria-da-divisao.md)** — dividir perde (169 → 37, **132 distinções**); a dualidade guarda a segunda metade. Sem involução não é dualidade, é degeneração — e o texto já o aplicava sem o enunciar.
...
- **[A escada aritmética ℕ→ℤ→ℚ](project-escada-aritmetica-n-z-q.md)** — ℤ dá a reversibilidade da SOMA, ℚ a da MULTIPLICAÇÃO não nula; e o gume dele na língua da casa: dividir por zero é uma operação SEM FIBRA. O andar seguinte é ℝ (os buracos).

## As realizações e o hardware

- **[A assistente: cone/espiral, e a CAIXA](project-assistente-cone-espiral.md)** — o desenho dele (involução na entrada, evolução no banco) e a fronteira MEDIDA: `3 x 3` desdobra, `3 vezes 3` não. O próprio ficheiro avisa porque alargá-la é perigoso.
...
- **[14/08 — O DIA LONGO, 36c1fa5 → 8dfb094](project-checkpoint-2026-08-14-curadoria.md)** — curadoria, migração para `lib/universal.js`, fases 3–6, a ronda das pontes, a tarde das palavras + a FUNDAÇÃO VETORIAL, a regressão total, O MARTELO, e à noite a ESCADA ARITMÉTICA. **468:468.** O ficheiro tem o dia por secções.
- **[13/08 tarde — O CRISTAL RECUPERADO, c0893a5](project-checkpoint-2026-08-13-cristal.md)** — o cristal grande estava no **broca-so** (75k registos → 4286 conceitos), 10 projeções, volta 13:0 com mutações REOPEN, banco ingere 4236 em 1,4s.
