...
- **[A asserção que passa sem poder falhar](feedback-assercoes-vazias.md)** — NOVE formas, e a nona é a pior: a tautologia escrita DENTRO da correcção, os dois lados copiados.
- **[A referência escrita à mão](feedback-a-referencia-escrita-a-mao.md)** — calculo a referência de cabeça e ESCREVO-a, reintroduzindo o defeito dentro da correção. Mudar o dado.
...
- **[O resultado verdadeiro e PARCIAL](feedback-verdadeiro-e-parcial.md)** — nenhum medidor o apanha porque a asserção está certa; o defeito é ter parado de perguntar.
- **[Dois caminhos que têm de concordar](feedback-dois-caminhos.md)** — os piores defeitos foram apanhados por COMPARAÇÃO, não por asserções. E ler o TOTAL: um medidor que não compila não falha.
- **[Representação inteligente](feedback-representacao-inteligente.md)** — a analiticidade é do OBJETO, não da representação. Medido: a série custa 98,8 ms e dá lixo; a forma fechada 0.
...
- [A insinuação está na arquitetura](feedback-insinuacao-arquitetonica.md) — álgebra certa, ressalvas presentes, e o texto insinuava. Se o resultado toca um problema famoso.
- [O que justifica a involução](feedback-justificar-o-que-so-e-coerente.md) — a conservação obriga, mas só onde é ADITIVA; nos corpos é multiplicativa (|N|=1). Erro repetido: números certos.
...
- **[A base incompleta](feedback-a-base-incompleta.md)** — um ponto fora do campo NÃO pede régua nova: faltava **metade do corpo** (`x† = −1/x`).
...
- **[O \medido sem medidor, e o seu DUAL](feedback-o-medido-sem-medidor.md)** — 28 blocos afirmam número sem nomear programa; e o `banco/fala.c` era citado só num .tex FORA da varredura.
...
- **[O medidor que enche o disco](feedback-o-medidor-que-enche-o-disco.md)** — o rodapé do `unidade.h` escreve para /tmp SEM TECTO: uma mutação em ciclo fez 8,2 GB e o `timeout` não desfaz.
- **[O teto não verificado](feedback-o-teto-nao-verificado.md)** — `an_zn(&R,40)` escreveu 1600 inteiros num array de 24×24 e a máquina deixou de terminar.
- **[O ramo que nunca corre](feedback-o-ramo-que-nunca-corre.md)** — mutação que sobrevive tem TRÊS causas: ramo inalcançável, guarda redundante, ou **o gume mal feito por mim** (não passa pelo sítio.
...
- **[Varrer onde nada pode falhar](feedback-varrer-onde-nada-pode-falhar.md)** — varredura num regime onde o defeito não vive: esforço alto sem indecisos, objeto SIMÉTRICO num teorema de assimetria.
- **[O destino rotativo](feedback-o-destino-rotativo.md)** — valor certo e TEXTO errado, invisível às asserções: `frac2` a rodar sob o printf, `%-Ns` por bytes, testemunha por estrear.
- **[Escrever numa notação e ler noutra](feedback-escrever-numa-notacao-ler-noutra.md)** — o `eval.txt` escreve «A + AB = A» e o parser exigia `*`: a fala morria CALADA.
...
- **[Definição e medida em gavetas diferentes](feedback-definicao-e-medida.md)** — a definição diz o que PODE ser preservado; a medida diz o que FOI.
...
- **[A mensagem que não pode falhar](feedback-a-mensagem-que-nao-pode-falhar.md)** — `cc …; echo "compilou"` imprimiu «compilou» com a compilação FALHADA, e o binário VELHO respondeu ao teste.
- **[O objecto que não cabe](feedback-o-objecto-que-nao-cabe.md)** — Σ1/n³ exato saiu NEGATIVO, e o defeito não era o guarda: era construir um objeto que não precisava de existir.
- **[A definição do extractor](feedback-a-definicao-do-extractor.md)** — publiquei «15 de 22 citam medidor»: era a definição da MINHA consulta, não um facto sobre o paper. Os 22 estavam medidos.
- **[O tecto do array](feedback-o-tecto-do-array.md)** — subi `TR_MAX` para 64 e chamei-lhe «não tem tecto». Se a conclusão menciona um número do meu código, medi a MÁQUINA.
- **[O double que só transportava](feedback-o-double-que-so-transportava.md)** — 20 doubles sobre dados que SEMPRE foram inteiros: não carregavam vírgula, traziam um limiar `1e-9` de borla.
...
- **[O escopo da afirmação](feedback-o-escopo-da-afirmacao.md)** — escrevi «não ordenado» sem escopo NA PORTA DE ENTRADA, e o documento já provava o contrário.
...
- **[O laço que para no primeiro](feedback-o-laco-que-para-no-primeiro.md)** — a busca por EXISTÊNCIA decide e não mede: de 91 pares varria UM. Quando um gume não morde.
...
- **[O contador que mudou de significado](feedback-mudar-o-significado-do-contador.md)** — `qz_saturou` passou de «truncou» a «promoveu» e SEIS medidores caíram, nenhum a falar de saturação.
- **[O que está no disco e não no git](feedback-o-que-esta-no-disco-e-nao-no-git.md)** — a bateria verde sobre uma árvore que o git não reproduz: `banco/*` sem `!banco/*.h`. Medida em refs.c §R5.
...
- **[A dualidade é a memória da divisão](project-dualidade-memoria-da-divisao.md)** — dividir perde (169 → 37, **132 distinções**); a dualidade guarda a segunda metade. Sem involução é degeneração.
...
- [A transformada universal](project-transformada-universal.md) — o corpo é AUTODUAL; mas dois revisores derrubaram metade: a diagonalização pela borda era FALSA (é avaliação nas FOLHAS de Frobenius).
- [Hopfield e as duas torres](project-hopfield-torres.md) — B_s tem período 2 e espelha (o J), B_a tem período 4 e roda (o i). Hadamard É a dobra. Com FORMA FECHADA.
- [A escada do diabo](project-escada-do-diabo.md) — 1/2 é o patamar mais largo; e um revisor apanhou DUAS frases falsas publicadas, incluindo «os convergentes do áureo encolhem mais depressa».
...
- [O corpo quântico e o cósmico](project-quantico-cosmico.md) — [σi,σj]=2i·ε·σk É o produto cruzado. E: CHAMEI LEI À CONSEQUÊNCIA DE UMA ESCOLHA MINHA (Carnot).
...
- **[A CONTINUIDADE nas duas direcções](project-continuidade-duas-direccoes.md)** — a casa media só «o habitante É o supremo da sua classe»; a volta — dado S limitado.
...
- **[A CÚSPIDE é o trial](project-a-cuspide-e-o-trial.md)** — `disc = 0` é onde as folhas colidem; τ = sign(disc) É o trial e indexa a dimensão τ+1.
...
- **[A derivada exterior](project-a-derivada-exterior.md)** — o `d` é UMA operação (grad/rot/div são nomes dela) e Green/Stokes/Gauss são UM teorema: a prova é o PROGRAMA ser um só.
- **[Cálculo II e III](project-calculo-2-3.md)** — a série é o OBJECTO e o valor é que precisa do limite; local→global; e Green/Stokes/Gauss são UMA frase: ∫_∂R ω = ∫_R Dω.
- **[Cálculo I exacto](project-calculo-exacto.md)** — `(f(a+h)−f(a))/h` **É** um polinómio em h, logo a derivada é uma AVALIAÇÃO e não um limite; e a casa já derivava exacto pela parte ε do dual.
...
- **[A DESCIDA: ℚ são classes de pontos fixos](project-descida-racionais-pontos-fixos.md)** — o TRAÇO projecta a família toda em ℤ (a norma é constante e não separa).
...
- **[A casa já corria Cayley–Hamilton](project-a-casa-ja-corria-cayley.md)** — a `estaca` (A·(mI−A)=−I) É ele, e as duas cartas são as duas formas quadráticas: o andar trouxe o NOME, não o motor.
- **[O gume automático](project-gume-automatico.md)** — retirar a hipótese e PROCURAR o contra-exemplo: a regra que eu aplicava à mão virou função.
...
- **[O deploy sem o GitHub](project-deploy-sem-github.md)** — quando o Actions cai, o runner local NÃO resolve. Resolve o `rsync` por SSH (o `--exclude repo.git` caducou a 20/08: o espelho saiu).
- [A publicação na Patria](project-publicacao-patria.md) — no ar, dois workflows cruzados pelo R2, NENHUM binário no git. **O repo é PRIVADO e o fork da Patria saiu da jogada (20/08).**
...
- **[17/08 — O ESPAÇO E O SEU DUAL](project-checkpoint-2026-08-17-o-espaco.md)** — `Q(m√D)` com as QUATRO propriedades (duas de cada lado); a transformada é a passagem à BASE DO DUAL.
- **[16/08 — A RAIZ SAI POR TEOREMA, a8ed821 → 2298d0f](project-checkpoint-2026-08-16-a-raiz-sai.md)** — o cruzado invariante a potências (e a perpendicularidade NÃO atravessa), ℤ[√D] na lib.

- **[14/08 — O DIA LONGO, 36c1fa5 → 74f4139](project-checkpoint-2026-08-14-curadoria.md)** — curadoria, `lib/universal.js`, fases 3–6, a ronda das pontes, a FUNDAÇÃO VETORIAL, O MARTELO.
- **[13/08 tarde — O CRISTAL RECUPERADO, c0893a5](project-checkpoint-2026-08-13-cristal.md)** — o cristal grande estava no **broca-so** (75k registos → 4286 conceitos), 10 projeções.
...
- **09/08–11/08** — [as 8 leis fecham a torre](project-checkpoint-2026-08-09-oito-leis.md), 386:386 · [a ronda do gabarito](project-checkpoint-2026-08-10.md) · [a composição dinâmica](project-checkpoint-2026-08-11.md), `cf30baa`
- **[09/08 — O PONTO FIXO DERIVADO](project-checkpoint-2026-08-09.md)** — o bit **é `i`** (`ν(x)=−1/x`), interfaces são as dobras `n²+4`; dicionário do milénio honesto (cada um sobre teorema PROVADO).
...
- **05/08** — [FECHO/no ar 303:303](project-checkpoint-2026-08-05-fecho.md) · [auto-contido](project-checkpoint-2026-08-05-auto-contido.md) · [relógio canónico: estrutura=relógio, réguas=números](project-checkpoint-2026-08-05-relogio-canonico.md) · [sem memória
