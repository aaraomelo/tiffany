...
equivalência diz que se está a fazer.}\item[(2)] \textbf{$\partial$ é involutiva, e tem um ponto fixo: o MEIO.} São
...
em largura $w$ há $2^{w}$ tuplas, e mais nada. \emph{É a conta, e é toda}. (A
§\ref{fis:mult}, a seguir, dá nome ao que aqui se conta --- a \textbf{dobra},
medida por $G$ --- e não é preciso esperar por ela para ver a cláusula.)
\end{enumerate}
\end{teorema}
...
onde o processo o pôs; a aplicação de um para o outro não é forçada a ser
injetiva por coisa nenhuma na construção. (3) É a contagem, e só ela: em largura
$w$ há $2^{w}$ tuplas, pelo que representar mais do que $2^{w}$ objetos obriga
duas tuplas a caírem no mesmo --- casa de pombos, sem mais.
\end{proof}
...
\midrule
a soma em $X_1$ \emph{(sem quoc.)} & $(x,y)\mapsto x\oplus y$ & a da soma & $x\oplus z$ & o \textbf{neutro} $0$ \\
...
que $\Zc$ não faz}. O fator fica pendurado, e o Teor.~\ref{fis:thm:H} diz onde.\end{definicao}
...
\begin{definicao}[bijeção, isomorfismo, duomorfismo]\label{fis:def:duomorf}
Sejam $A=(X,\oplus_1,\otimes_1)$ e $B=(Y,\oplus_2,\otimes_2)$ estruturas com
\emph{duas} operações cada --- e as operações são \textbf{dados da estrutura},
não a soma e o produto: podem ser $\min$ e $\max$, as do
Teor.~\ref{fis:thm:B}, ou quaisquer duas que a construção produza. Para
$f\colon X\to Y$:
\[
\begin{array}{ll}
\textbf{bijeção} & \text{correspondência reversível, e nada mais;}\\[3pt]
\textbf{isomorfismo} & f(x\oplus_1 y)=f(x)\oplus_2 f(y)\ \ \text{e}\ \ f(x\otimes_1 y)=f(x)\otimes_2 f(y);\\[3pt]
\textbf{duomorfismo} & f(x\oplus_1 y)=f(x)\otimes_2 f(y)\ \ \text{e}\ \ f(x\otimes_1 y)=f(x)\oplus_2 f(y).
\end{array}
\]
Isto é: o duomorfismo é o isomorfismo para a estrutura com as operações
\emph{trocadas},
\[
\boxed{\;f\colon\ (X,\oplus_1,\otimes_1)\ \xrightarrow{\ \cong\ }\ (Y,\otimes_2,\oplus_2)\;}
\]
\emph{O «duo» não vem de haver dois objetos} --- vem da dualidade operacional
$\oplus_1\leftrightarrow\otimes_2$, $\otimes_1\leftrightarrow\oplus_2$. E onde
houver ordem ela vai junto: a troca obriga $f$ a ser \textbf{antitónica},
$a\le_1 b\iff f(b)\le_2 f(a)$, e a chegada é $\le_2^{\mathrm{op}}$.
\end{definicao}
...
\medskip\noindent\textbf{As três colunas, lado a lado.}
\begin{center}\small
\begin{tabular}{@{}llll@{}}
\toprule
...
ordem & --- & $\le_1\mapsto\le_2$ & $\le_1\mapsto\ \ge_2$ \\
...
$\abs{I}$, mais matéria é menos espaço}. É a antitonia que a
Def.~\ref{fis:def:duomorf} pede quando há ordem.
...
operação trocada e a ordem invertida, e com um número --- o $c$ --- a passar de
uma para a outra com o sinal virado. \emph{A igualdade seria colapso; a duomorfia
...
que falta é ela ter voz no passo.}
...
\end{proof}
\colunas{Lema~\ref{fis:lem:ultra}; Teor.~\ref{fis:thm:bolas},~\ref{fis:thm:euler},~\ref{fis:thm:dim},~\ref{fis:thm:vizniso},~\ref{fis:thm:navega},~\ref{fis:thm:duasinvol},~\ref{fis:thm:dual},~\ref{fis:thm:constroi},~\ref{fis:thm:duoesp},~\ref{fis:thm:ncorpos},~\ref{fis:thm:tresgraus},~\ref{fis:thm:ponte},~\ref{fis:thm:palavra},~\ref{fis:thm:handshake}; Lema~\ref{fis:lem:doisunico},~\ref{fis:lem:quatro}; Cor.~\ref{fis:cor:folga},~\ref{fis:cor:travunica},~\ref{fis:cor:cruzado}; Def.~\ref{fis:def:espmat},~\ref{fis:def:corpos},~\ref{fis:def:palavra}}
...
empresta quatro, e cinco não estão aqui --- e enumerar quais é mais honesto do que
...
momento & só o \emph{nome} & Teor.~\ref{fis:thm:volta}: uma passagem \\
energia & noutra parte & no grau dois --- Teor.~\ref{fis:thm:tresgraus} \\
força & \textbf{não}, no espaço & não há fonte --- Teor.~\ref{fis:thm:semfonte} \\
...
$\partial^{2}=\mathrm{id}$ e mais nada: não há grandeza aditiva entre corpos ---
não há corpos ---, nem grandeza que se conserve ao longo do parâmetro. Vale aqui
a mesma frase da §\ref{fis:rel-naoderiva}: \emph{o que se deriva é o limite; o
que se atribui é o nome.}
...
§\ref{fis:grav-regua}, e o que \textbf{puxa} não está em parte nenhuma.}\ }
\]
...
que a construção tem é o \emph{outro}, e conta-se qual.
...
\medskip\noindent\textbf{E é por isso que o que aqui há não é atracção.} Um poço
...
\medskip\noindent\textbf{Para ter atracção há um passo, e ele custa.} Basta
trocar o passo~3 para o vizinho de \emph{maior} $G$. Isso é uma
\textbf{realização nova} --- muda o ciclo da §\ref{fis:algoritmo} ---, e paga-se
com o que qualquer realização paga: enunciado próprio e medidor próprio.
\emph{Nenhum dos dois está escrito, e por isso ela não entra aqui.}
...
\noindent Quatro coisas, e dizem-se porque um leitor treinado em gravitação vai
supor as quatro sem ler.

\medskip\noindent\textbf{Primeira: a atracção.} O sinal é o outro, e o
Teor.~\ref{fis:thm:sinal} conta-o nas duas leituras. Nada aqui puxa.

\medskip\noindent\textbf{Segunda: a fonte, e com ela a lei de força \emph{no
espaço}.} Não há $T_{uv}$ e não há termo que faça um constituinte mover-se por
causa de outro --- Teor.~\ref{fis:thm:semfonte}. \emph{E é preciso ser exacto
...
\medskip\noindent\textbf{Terceira: a massa, e a ponte com ela.} A palavra está
...
\medskip\noindent\textbf{Quarta: a dilatação do tempo, e ela vem ao contrário.}
...
\[
\boxed{\ \text{o que se deriva é a \textbf{régua} e a \textbf{queda}; o que não se deriva é o \textbf{puxão}.}\ }
\]
...
\colunas{Teor.~\ref{fis:thm:gcanon},~\ref{fis:thm:campo},~\ref{fis:thm:curvatura},~\ref{fis:thm:gradiente},~\ref{fis:thm:magnitude},~\ref{fis:thm:divergente},~\ref{fis:thm:materia},~\ref{fis:thm:geodesica},~\ref{fis:thm:sinal},~\ref{fis:thm:semfonte}; Cor.~\ref{fis:cor:encontro}; Def.~\ref{fis:def:curvatura},~\ref{fis:def:grad},~\ref{fis:def:materia}}
        {a régua e a sua diagonal; o campo que a curva; a queda; o sinal trocado; a ausência de fonte}
        {\code{tests/aranha\_n.c} §AN21--§AN22, §AN28, §AN31}
...
decide é $2p=q$ --- \textbf{divisibilidade}, num anel onde o $2$ nunca se
...
\emph{Hipótese importada, e a única desta obra}: apagar um bit tem um custo
mínimo, e ele é positivo. Sob ela:
\begin{enumerate}\itemsep3pt
...
o que faltava ao par.} $I_2$ é a \textbf{dualidade}: não toca no destino e troca
o que dilui pelo que concentra. $I_1$ é o \textbf{destino}: troca travar por
acelerar e não toca na dualidade. \emph{São independentes porque fazem coisas
diferentes, e é isso que o grupo de Klein regista.}
