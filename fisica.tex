...
%
%  O QUE ESTE FICHEIRO É: a matéria da aranha estigmérgica ORGANIZADA nos onze
%  lugares onde ela opera — álgebra, topologia, análise, geometria, mecânica,
%  relatividade, óptica, partículas e núcleo, termodinâmica, cosmologia, redes. Nada foi
...
\ifSubfilesClassLoaded{%
  \gkcapa{A Física Araniana}
         ...
         {álgebra, topologia, análise, geometria, mecânica, relatividade, óptica,
          partículas e núcleo, termodinâmica, cosmologia, redes --- onze lugares, um sistema}
  \maketitle
  \tableofcontents
}{%
...
opera em \textbf{onze} lugares, e é por eles que ela se reparte aqui. A
...
curva. A \textbf{relatividade} conta o que um passo alcança: o limite que a
...
\noindent Uma definição que \emph{postulasse} a bijeção duomorfa não valeria nada
aqui --- seria uma escolha, e a Def.~\ref{fis:def:op} não permite escolhas. O que
segue constrói-a, e a construção tem um só passo: \textbf{sobre dois objetos o
...
\colunas{Lema~\ref{fis:lem:ultra}; Teor.~\ref{fis:thm:bolas},~\ref{fis:thm:euler},~\ref{fis:thm:dim},~\ref{fis:thm:vizniso},~\ref{fis:thm:navega},~\ref{fis:thm:duasinvol},~\ref{fis:thm:constroi},~\ref{fis:thm:dual}; Lema~\ref{fis:lem:doisunico},~\ref{fis:lem:quatro}; Cor.~\ref{fis:cor:folga},~\ref{fis:cor:travunica}}
...
objeto, a face tem meio; se não tem, a face não tem meio} --- e as duas
alternativas são a cláusula~(3) do Teor.~\ref{fis:thm:corte}, decididas por um
produto. É a mesma economia do traço da família real: de $\sigma+\sigma'=m$ e
...
$\delta_0$. OLema~\ref{fis:lem:descida} dá $\log_{2}N$, e ainda \textbf{sem a raiz}: a face
...
$g\,g=ab=m^{2}-\delta^{2}\le m^{2}$ com igualdade só em $\delta=0$ edesigualdade \emph{estrita} fora dele ($\mathbf{1890}$ pares); encaixa e halva em
...
ele usa é da \textbf{representação} e não do teorema}. Oencaixotamento certificado está em \code{tests/agm.c} §A1--§A3, e a cota
...
Não se postula o intervalo: é a forma quadrática que a Def.~\ref{fis:def:tempo}
já escreveu. Não se postulam três classes: é o sinal dessa grandeza, que a régua
...
o intervalo & $q=g_{uv}\partial x^{u}\partial x^{v}$, inteiro na aresta se $h$ o for & Def.~\ref{fis:def:tempo}(ii) \\
...
\noindent A Mecânica entrega tudo o que segue, e nenhuma outra parte o tem: o
\textbf{tempo} como contagem de passos (Def.~\ref{fis:def:tempo}), a
\textbf{forma quadrática} $q$ com $g$ simétrica e não degenerada, a \textbf{régua
curvada pelo campo} $g_{uv}(x)=G(x)h_{uv}$, e a \textbf{queda}
(Teor.~\ref{fis:thm:geodesica}: \emph{a aranha não decide --- cai}). A
§\ref{fis:tempo} chama-se «O espaço e o tempo»; esta parte é a sua continuação
directa.
...
forma quadrática da Def.~\ref{fis:def:tempo}(ii), que a §\ref{fis:rel-intervalo}
...
\noindent A Def.~\ref{fis:def:tempo}(ii) já escreveu o intervalo, e a
...
descida infinita exclui-o; com $b=0$ vem $a=0$. A caracterização por $\Delta$
quadrado é o Cor.~\ref{fis:cor:borda}, já demonstrado.

(2) $q$ é quadrática: $q(\lambda v)=\lambda^{2}q(v)$, que é a
Def.~\ref{fis:def:tempo}(ii) lida sobre um múltiplo. Com $q(\partial x)=0$ o lado
...
verificada na Mecânica e aqui usada sem se repetir a conta. \emph{E fica dito o
...
que éonde o processo \textbf{chega} --- e aí as duas faces partilham o ponto fixo. Se
...
\emph{O determinante.} Se fosse $\det A=-1$, o Cor.~\ref{fis:cor:optica} daria
$\Delta=B^{2}+4\ge4>0$, contra $\Delta<0$. Logo $\det A=+1$ e $\Delta=t^{2}-4$
...
característico; a factorização existeexactamente quando as raízes estão em $A$. A condição sobre as metades é a mesma
...
identidade $g\,g=ab=m^{2}-\delta^{2}=(m+\delta)(m-\delta)$ \emph{é} a diferençade quadrados, e daí as duas bordas saem em forma fechada:
\[
\Delta=0:\ q=(2m)^{2}
\qquad\qquad
\Delta=4:\ q=4\,m\,\delta .
...
Teor.~\ref{fis:thm:corte}(5) já tinha identificado como o sítio onde o encaixedas duas faces \textbf{chega}.
\[
...
produza sem sair dos inteiros. Os pares conjugados são ainvolução do Teor.~\ref{fis:thm:duas}(2) a agir no espectro: $\theta$ e $-\theta$
...
Seja $c$ uma cadeia de grau $N$ com $\abs{c_0}=1$ e $C$ a sua companheira. Aórbita de $e_1$ sob $C$ --- que é uma sucessão de vectores \emph{inteiros} ---
...
\medskip\noindent\textbf{E é isto que a tabela do §E3 estava a dizer.} O $5$
...
(Teor.~\ref{fis:thm:corte}(3)). O da conjugação é $Q=0$, eesse \textbf{é} alcançado: é o estado neutro, o \emph{vácuo} do ideal --- o único
...
amostra fecha o Cor.~\ref{fis:cor:irrever} ao valor exacto. E overedito \textbf{negativo} sobre a razão focal está no próprio medidor, não numa
nota de rodapé.}
...
que o corte}. \emph{O tecto diz-se no tipo}, e um número publicadoerrado é pior do que um bloco vermelho.}
...
\section{A curvatura, e o que ela é neste documento}\label{fis:curvatura}

\noindent Fora da linha do roteiro, a palavra ainda não tinha sido usada --- e há
razão: ela já estava escrita sem nome. A §\ref{fis:tempo} pôs
\[
g_{uv}(x)=G(x)\,h_{uv} ,
\]
e disse o que isso significa --- \emph{é a memória, ao ficar no espaço, que o
curva}. Não é preciso importar curvatura nenhuma: ela é o campo $G$, e o que
falta é lê-la nas coordenadas que esta parte já tem.
...
\colunas{Teor.~\ref{fis:thm:taxa},~\ref{fis:thm:conteudos},~\ref{fis:thm:diluicao},~\ref{fis:thm:bidual},~\ref{fis:thm:conica},~\ref{fis:thm:curvatura}; Cor.~\ref{fis:cor:destino},~\ref{fis:cor:destino-orbita},~\ref{fis:cor:fronteira-curv}}        {$H^{2}=1/D$; os três conteúdos no trial; a diluição com a sua constante; a órbita de quatro; a curvatura como dobra, e a fronteira que não é estado}
