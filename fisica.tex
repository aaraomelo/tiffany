...
$ab/x=x$ tem a única solução positiva $x=\sqrt{ab}$ --- e ela existe no degrau
$X_4$ da §\ref{fis:escada}, e em geral \emph{não} nos anteriores: é este ponto
fixo que obriga a subir de andar. \emph{E é por isso que o piso da
Def.~\ref{fis:def:piso} faz falta}: sem ele o processo sai do objeto que o
constrói. Isto vale sempre que a raizse inverta --- a mesma condição que o $2$ era na face aditiva.
\end{proof}
...
\item[(5)] \textbf{e o ponto fixo comum é exacto só onde $\delta=0$.} Os pontos
fixos \emph{exactos} das duas faces são $m_{\ast}=\tfrac{a+b}{2}$ e
$g_{\ast}=\sqrt{ab}$, e coincidem \textbf{se e só se} $a=b$ --- que é $\delta=0$
no Cor.~\ref{fis:cor:medias}. Na grade o processo acaba com $m_N=g_N$, e é isso
que a régua consegue dizer; \emph{mas isso não obriga $a_N=b_N$}, e convém não
colar as duas coisas: com $a=4$ e $b=5$ vem $m=\lfloor\tfrac92\rfloor=4$ e
$g=\lfloor\sqrt{20}\rfloor=4$, e as pontas continuam distintas. \emph{A régua faz
coincidir o que a grade não separa}, e o que fica dito do ponto é que ele está no
grão: $g_N\le g_{\ast}<g_N+u$. \emph{O corte não é um ponto que se ache entre as
classes --- é o ponto que as duas faces passam a partilhar, exactamente quando
$\delta=0$ e à resolução da régua sempre.}\end{enumerate}
...
(5) Os dois pontos fixos exactos são $\tfrac{a+b}{2}$ e $\sqrt{ab}$, e coincidem
sse $(a+b)^{2}=4ab$, isto é sse $(a-b)^{2}=0$, isto é sse $a=b$. Aí, com $a=b=p$,
vem $\partial^{+}p=p+p-p=p$ e $\partial^{\times}p=p^{2}/p=p$: o ponto é fixo pelas
duas ao mesmo tempo e sem resíduo. \emph{Que a igualdade na grade não devolve a
igualdade exacta} é a testemunha do enunciado, e é uma conta de dois piso; e
$g_N\le g_{\ast}<g_N+u$ é a Def.~\ref{fis:def:piso} aplicada ao último par.\end{proof}

\begin{lema}[a largura halva, e à face geométrica só se pede que não saia]\label{fis:lem:descida}
Seja $w=m-g$ a largura em grãos e $(m',g')$ a batida seguinte, com $m'$ o piso de
$\tfrac{m+g}{2}$ e $g'$ \emph{qualquer} elemento da grade com $g\le g'\le m'$.
Então
\[
\boxed{\;m'=m-\Bigl\lceil\frac{w}{2}\Bigr\rceil
\qquad\text{e}\qquad
w'\;\le\;\Bigl\lfloor\frac{w}{2}\Bigr\rfloor\;}
\]
\emph{E nada disto olha para a raiz.} A única coisa pedida à face geométrica é
que o seu valor \textbf{fique no intervalo} --- não o seu tamanho, não a sua
identidade. \emph{A largura halva por causa da face aditiva; a geométrica só tem
de não sair.}
\end{lema}

\begin{proof}
Escreva-se $g=m-w$; então $\tfrac{m+g}{2}=m-\tfrac{w}{2}$, e o maior ponto da
grade que não passa esse valor é $m-\lceil w/2\rceil$. Com $g'\ge g$,
\[
w'=m'-g'\;\le\;m-\Bigl\lceil\frac{w}{2}\Bigr\rceil-g
\;=\;w-\Bigl\lceil\frac{w}{2}\Bigr\rceil\;=\;\Bigl\lfloor\frac{w}{2}\Bigr\rfloor .
\]
E para $w\ge1$ vem $\lceil w/2\rceil\ge1$, isto é $m'\le m-u$: a descida é
estrita enquanto houver largura.
\end{proof}

\begin{corolario}[a cota é logarítmica, e ainda não usa a raiz]\label{fis:cor:cota}
Se a largura inicial tem $N$ grãos, o processo chega à largura zero ao fim de
\[
\boxed{\;\bigl\lceil\log_{2}N\bigr\rceil\ \text{batidas}\;}
\]
--- e não $N$. A cota sai do Lema~\ref{fis:lem:descida} sozinho, e portanto vale
para \emph{qualquer} segunda face que devolva um ponto do intervalo.
\end{corolario}
...
a entropia do lado negro cresce em todos os passos; a do branco decresce emtodos; e a \textbf{soma não se move em nenhum}. Logo
\[
...
\colunas{Teor.~\ref{fis:thm:carnot},~\ref{fis:thm:landauer},~\ref{fis:thm:entropiadual}; Cor.~\ref{fis:cor:irrever},~\ref{fis:cor:carnothom},~\ref{fis:cor:custofolga},~\ref{fis:cor:segundalei}}        {o $\oint$ fechado; o piso zero do involutivo; o par que fecha na unidade}
