...
\begin{definicao}[o que o par carrega, e a raiz nunca é avaliada]\label{fis:def:piso}
O processo \textbf{não arredonda coisa nenhuma}, e convém dizê-lo antes de
...
\end{corolario}
\begin{proof}
Por indução, $w_n\le\lfloor N/2^{n}\rfloor$: o caso $n=0$ é a hipótese, e do
Lema~\ref{fis:lem:descida} vem
$w_{n+1}\le\lfloor w_n/2\rfloor\le\bigl\lfloor\lfloor N/2^{n}\rfloor/2\bigr\rfloor
=\lfloor N/2^{n+1}\rfloor$. Com $2^{n}\ge N$ vem $w_n=0$.
\end{proof}

\medskip\noindent\textbf{A cota, e o que cada face acrescenta.} São \emph{três}
números, e não dois. A descida ingénua --- um grão por batida --- dá $N$. O
Lema~\ref{fis:lem:descida} dá $\log_{2}N$, e ainda \textbf{sem a raiz}: a face
