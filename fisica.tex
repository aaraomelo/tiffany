...
por cima.}
\item[(4)] \textbf{E a caixa classifica-a: o duomorfismo \emph{reflete}, não
roda.} A cifra de $S$ é $\operatorname{tr}=0$ e $\det=-1$, donde
$\Delta=0+4=4>0$: pelo Teor.~\ref{fis:thm:leidisc} é \textbf{hiperbólica}, e pelo
Cor.~\ref{fis:cor:optica} é \textbf{imprópria}. \emph{É por isso que ela troca os
eixos em vez de os girar} --- e é a separação entre os dois: o que fecha
\emph{rodando} é o rotor do Teor.~\ref{fis:thm:fecho}, com $\det=+1$; o que fecha
\emph{reflectindo} é esta, com $\det=-1$. \emph{Nada disto foi escolhido: sai da
cifra de uma matriz de troca.}
...
$\det S=0\cdot0-1\cdot1=-1$. Logo $\Delta=\operatorname{tr}^{2}-4\det=4$, e o
Teor.~\ref{fis:thm:leidisc} põe-na na classe hiperbólica --- o que também sai do
Cor.~\ref{fis:cor:optica} sem olhar ao traço, porque $\det=-1$ dá
$\Delta=B^{2}+4\ge4$ seja qual for $B$.
...
\medskip\noindent\emph{Um aviso de letra, e vale para toda esta parte}: nesta
secção $q$ é o \textbf{número de marcas} da régua fechada, e na
§\ref{fis:rel-intervalo} $q$ é a \textbf{forma quadrática} da
Def.~\ref{fis:def:tempo}(ii). São dois objetos e uma só letra --- o documento já
gasta $B$ no binário e $\mathcal{B}$ na bola pela mesma razão. \emph{Onde as duas
aparecem juntas, o contexto di-lo, e onde não disser é defeito.}

\begin{teorema}[o máximo é a involução]\label{fis:thm:meiavolta}
Sobre uma régua fechada de $q$ marcas, a distância entre duas marcas separadas de
...
\begin{enumerate}\itemsep3pt
\item[(1)] \textbf{O máximo é $q/2$, e é atingido exactamente onde $p=q-p$.}
\item[(2)] \textbf{E esse ponto é o ponto fixo da dobra.} A aplicação
$p\mapsto q-p$ é a Def.~\ref{fis:def:op} com extremos $0$ e $q$; o seu único
...
$p=q-p$ se e só se $q$ é \textbf{par}. Em $q$ ímpar a involução não tem ponto
...
$p=q/2$ ida e volta dão o mesmo número; o passo deixa de distinguir os dois
sentidos, e com ele deixa de haver o sucessor que a Def.~\ref{fis:def:percurso}
pede para haver composição.
\item[(5)] \textbf{O número muda com a régua; a razão não.} Trocando $q$
\emph{par}, o máximo $q/2$ muda de valor e a razão $d/q$ vale sempre
...
elas cruzam-se onde $p=q-p$, e aí $d=p=q/2$. Para $p$ menor vale $d=p<q/2$; para
$p$ maior vale $d=q-p<q/2$. Logo o máximo é $q/2$ e é atingido só no cruzamento.

(2) $\partial p=0+q-p$ é a Def.~\ref{fis:def:op} com $a=0$ e $b=q$; pelo
Teor.~\ref{fis:thm:duas}(2) ela é involutiva e tem um único ponto fixo, o meio
$\tfrac{q}{2}$. O cruzamento de~(1) é a equação $\partial p=p$, que é a definição
de ponto fixo. \emph{Não são dois factos: é um, lido em duas linguagens.}

(3) $p=q-p$ é $2p=q$. Com $q$ par a solução está na régua; com $q$ ímpar o lado
...
(4) Em $p=q/2$ tem-se $p=q-p$, isto é, os dois caminhos têm o mesmo comprimento;
nada na régua os separa. Sem sentido definido não há sucessor bem definido, e a
Def.~\ref{fis:def:percurso} pede um.

(5) $d(q/2)/q=\tfrac12$ para todo $q$ par, por substituição directa. \emph{A razão
é da dobra; o número é da régua.}
\end{proof}

\begin{corolario}[no máximo, o par de sentidos tem um elemento só]\label{fis:cor:semmassa}
Com $q$ par, no ponto $p=q/2$ os dois sentidos dão o mesmo número --- é a
cláusula~(4) --- pelo que o par $\{p,\,q-p\}$ tem \textbf{um} elemento e não dois.
...
\begin{proof}
$p=q-p$ dá $\{p,q-p\}=\{p\}$, de cardinal um. É o
...
inteiro}; só $\tau$ pede raiz. Tudo o que segue corre sobre $q$, e nenhuma raiz é
avaliada.
...
\midrule
régua fechada, $q$ par & $\operatorname{Fix}(\partial)$ está na régua & o máximo \textbf{atinge-se} --- e ali não há repouso \\
régua fechada, $q$ ímpar & $\operatorname{Fix}(\partial)=\emptyset$ & o máximo é aproximado e nunca alcançado \\
...
razão $d/q$ vale $\tfrac12$ em toda a escala --- o número muda com a régua e a
razão não, que é o Teor.~\ref{fis:thm:meiavolta}(5). §N2: e \emph{só fecha em $q$
par} --- em $q$ ímpar não há $p$ com $p=q-p$, isto é, a involução \emph{não tem
