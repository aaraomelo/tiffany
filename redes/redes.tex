...
%
%  P6 é instanciação de fis:thm:combina, não postulado. Hebb/Hopfield são declarações.
...
\gkcapa{Redes Neurais Multifocais}
       {P6 é instanciação, não postulado}
       {a rede só realiza; não legisla}
\maketitle

\begin{abstract}\noindent
Hebb/Hopfield na rede \textbf{não} são a fonte da lei, nem um postulado.
São declarações: $W_{ij}$ é o Cor.~\code{fis:cor:hebb}; a descida energética
é o Teor.~\code{fis:thm:desce}. A lei da combinação é o
Teor.~\code{fis:thm:combina}. A descida é
\[
...
superposição em $V$. O fecho das duas faces é a Lei~7 --- interface realizada
pelo duomorfismo (\code{fis:obs:octoniao-interface}); \emph{não} há Lei~8 nem
postulado neste documento. A autoridade não vem de $\operatorname{lexMax}$,
$D_{\mathrm{can}}$, $\tau$ nem do pipe.
\end{abstract}

% ═══════════════════════════════════════════════════════════════════════════
\section{Descida operacional}\label{redes:regra}

\begin{teorema}[\pt{6} --- a descida é teorema, não postulado]\label{redes:post:p6}\label{redes:post:hebb}\label{redes:post:niveis}\label{redes:post:overwrite}\label{redes:thm:p6}
...
\]
Hebb e Hopfield são \textbf{declarações} --- corolário e teorema da física ---,
não axiomas deste documento. Este documento \textbf{não legisla}: só realiza
o que a física já provou. O fecho das duas faces é a Lei~7 (Obs.~\code{fis:obs:octoniao-interface}):
interface realizada pelo duomorfismo; \emph{não} há Lei~8.
\end{teorema}

\begin{proof}
A combinação é o Teor.~\code{fis:thm:combina}; $W_{ij}$ é o
Cor.~\code{fis:cor:hebb}; a descida energética é o Teor.~\code{fis:thm:desce}.
Logo Hebb/Hopfield não reabrem postulado. As faces não fundem: a interface é o
duomorfismo, e o fecho é a Lei~7.
\end{proof}
...
\begin{proof}
Hebb é soma; soma de capacidade $1$ é substituição. Teor.~\ref{redes:thm:p6}.
\end{proof}
...
\;}
\]
\medskip\noindent\textbf{Donde a dualidade vem} --- cadeia positiva, descida;
este documento não legisla.
\[
\begin{array}{@{}cl@{}}
\text{Lei~7} & \text{octonião dual: dois tecidos, ligar sem fundir}\\[2pt]
& \text{(o oito é grau e cardinalidade; \emph{não} há Lei~8)}\\[2pt]
\to & \text{observador $\neq$ observado (as faces não fundem)}\\[2pt]
\to & \text{interface necessária}\\[2pt]
\to & \text{duomorfismo: $A\cong B^{\vee}$ (\code{fis:def:duomorf}; \code{fis:obs:octoniao-interface})}\\[2pt]
\to & R^{\dagger}\text{ do Teor.\ \code{fis:thm:B}}\\[2pt]
\to & D=R^{\dagger}\circ R^{-1}\text{ única}\\[2pt]
\to & D(i)=(N-1)-i=\partial i\\[2pt]
\to & \text{relógio lê }u=2^{-\operatorname{prof}}
\end{array}
\]
A hexal (seis) é a interface \emph{aritmética} da torre; o duomorfismo é a
interface \emph{operacional}. Distintos de $W_s/W_a$.
\emph{A interface não é escolhida pelo relógio; o relógio é uma leitura da
interface já realizada} (\code{fis:obs:octoniao-interface}).
\emph{A involução não escolhe o duomorfismo}: o duomorfismo realiza a assimetria;
$D=\partial$ aparece depois, das duas nomeações.
...
\]
com uma \textbf{única} realização $\pi\colon I\to X$ num mesmo espaço $X$.
Os quatro \emph{focos} --- leituras ou agentes --- são fontes dentro da mesma
realização cerebral, e não quatro redes separadas.

\emph{Não confundir} com a troca dual
$R,R^{\dagger}$ do \fis{fis:thm:troca-realizacao}{Teor. «troca de realização»}:
a isometria $u(Da,Db)=u(a,b)$ exige aquelas leituras
(Prop.~\ref{redes:prop-troca}).
...
(\code{fis:cor:hebb}; Teor.~\ref{redes:thm:p6} é a descida, não o decreto).
...
(\code{fis:cor:hebb}; Teor.~\ref{redes:thm:p6}). Overwrite no
...
\medido{\code{tests/orbitas\_hopfield.js}: padrões $\xi$ das órbitas
\code{sql}/\code{latex}/\code{node} (corpus \code{idioma/pt/orbita} e face
\LaTeX{} dual); Hebb $W_s$; canal $W_a$; $\lambda^{+}+\lambda^{-}=0$.
Eixos $W_s/W_a$ e duomorfismo permanecem distintos. A fita continua $G=1$.}

\medskip\noindent\textbf{Hopfield não é teoria do transporte.} A cadeia
correcta no motor cliente/servidor é
\[
\text{arena}\to\text{duas faces}\to\text{absorção}\to\text{órbitas}
\to\xi\to W.
\]
HTTP/\texttt{POST}/\texttt{FETCH} ficam \code{fios}: transporte externo,
\emph{não} memória, \emph{não} órbita. O que virou órbita foi o que foi
absorvido. $W_a$ ordena o canal das línguas absorvidas (período~$4$);
\emph{não} é o fio HTTP, nem a aresta $a{=}1$.
\caixa{a \textbf{matriz} memoriza as línguas absorvidas; o \textbf{fio}
transporta o que ainda não foi absorvido}\label{redes:memoria-fio}

\medskip\noindent\textbf{Descida do domínio} (\code{fis:def:dominio-interface}).
Não se transformam os quatro quadrantes em quatro operadores. A leitura
$2{\times}2$ é a interface \emph{filtrada}:
\[
\mathcal{M}=\mathcal{O}\sqcup\mathcal{T},\qquad
\mathcal{D}^{2}=\mathrm{id}_{\mathcal{O}},
\]
com $\mathcal{O}=\{\texttt{sql},\texttt{latex},\texttt{node},\ldots\}$ (órbitas)
e $\mathcal{T}=\{\texttt{post},\texttt{http},\texttt{fetch}\}$ (\code{fios}).
$\mathcal{D}$ troca a face \emph{dentro} de $\mathcal{O}$; em $\mathcal{T}$ não
tem domínio. O canto inferior direito é vazio porque o fio ainda não tem
$\oplus/\otimes$.
\[
\begin{array}{c|cc}
 & a{=}0 & a{=}1\\\hline
\mathcal{O} & \xi,\;Q_{00/01} & \xi^{\vee},\;Q_{10/11}\\[2pt]
\mathcal{T} & \text{fio} & \text{---}
\end{array}
\]
Dentro de $\mathcal{O}$ entram os dois bits que já existem,
$Q_{ab}=\mathcal{D}^{a}\dagger^{b}$. $W=W_s+W_a$ é a matriz \emph{depois} da
absorção, noutro eixo --- \emph{não} é a linha $a{=}1$. $G=1$ continua a ser a
fita. A frase seguinte deixa de ser só editorial: é a Def.~\code{fis:def:dominio-interface}
lida no pipe.
...
peça & faz & não é \\
\midrule
$W_s$ ($W_s^{\top}=W_s$) & memória --- recupera $\xi$ & transporte \\
$W_a$ ($W_a^{\top}=-W_a$) & ordem do canal --- $s^{\top}W_as=0$ & aresta $a{=}1$, nem face~$1$ \\
$Q_{ab}$ & quarteto interno a $\mathcal{O}$ & quatro operadores sobre $\mathcal{M}$ \\
$G{=}1$ & fita / passagem única & Hopfield \\
$\mathcal{T}$ & HTTP não absorvido & órbita, $W$, $\mathcal{D}$ \\
\bottomrule
\end{tabular}
\end{center}
...
interferência & \code{fis:thm:combina}, \code{fis:cor:hebb} & \pt{6} descida & $W$ nas órbitas (\code{orbitas\_hopfield.js}); fita $G=1$ \\
recuperação & \code{fis:thm:desce} & âncora / Hopfield dual & $\lambda^{+}+\lambda^{-}=0$ \\
$W_s/W_a$ & \code{fis:thm:duastorres} & \code{mf:thm:duas} & \code{orbitas\_hopfield.js}; \code{hopfield.c} \\
duomorfismo & \code{fis:def:duomorf} & --- & \code{wasm\_erg}, MOVE (III) \\
domínio & \code{fis:def:dominio-interface} & --- & \code{cat:dominio-interface}: $\mathcal{O}/\mathcal{T}$ \\
fio HTTP & --- & --- & \code{fios} --- transporte; \emph{não} $W$ \\
\bottomrule
\end{tabular}}
\end{center}
...
\textbf{absorção} & órbita / fio & memória das línguas / transporte HTTP \\
\bottomrule
\end{tabular}
\end{center}
...
--- \emph{fora} da bateria RG; \code{tests/orbitas\_hopfield.js} mede $W_s/W_a$ nas
órbitas absorvidas, \emph{não} nos \code{fios}.

\caixa{interface não é transporte; memória não é fio}\label{redes:interface-fio}
--- $\mathcal{D}$ interno a $\mathcal{O}$; $\mathcal{T}$ só transporta;
$W$ só memoriza $\mathcal{O}$.
...
\code{tests/orbitas\_hopfield.js} (órbitas sql/latex/node; \code{fios} fora de $W$);
...
Teor.~\ref{redes:thm:p6}, Cor.~\ref{redes:cor:ow},
