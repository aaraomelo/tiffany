...
\begin{proposicao}[Scores finitos são racionais; no limite pode surgir um irracional]
\label{prop:score-limite}
Seja \(S=\{s_0,\dots,s_m\}\subset\mathbb Q\) com \(s_0<\dots<s_m\). Para toda
realização finita não vazia \(I\),
\[
\boxed{
\mathbb E_G[S]
=
\frac{\sum_{i=0}^{m}s_i\,G_q(s_i)}{\lvert I\rvert}
\in\mathbb Q.
}
\]
Por outro lado, uma sequência destes scores pode convergir para um número
irracional. Fixemos \(\alpha\in(0,1)\setminus\mathbb Q\) (p.\,ex.\
\(\alpha=\sqrt2/2\)) e \(S=\{0,1\}\). Para cada \(n\ge1\), tomemos uma
realização \(I_n\) finita com \(\lvert I_n\rvert=n\) e
\(G_{q,n}(1)=\lfloor\alpha n\rfloor\),
\(G_{q,n}(0)=n-\lfloor\alpha n\rfloor\). Então
\[
\mathbb E_{G_n}[S]=\frac{\lfloor\alpha n\rfloor}{n}\in\mathbb Q,
\qquad
\lim_{n\to\infty}\mathbb E_{G_n}[S]=\alpha\notin\mathbb Q.
\]
\end{proposicao}

\begin{proof}
A primeira parte é Def.~\ref{def:q-score} e Def.~\ref{def:esperado}:
\(s_i\in\mathbb Q\) e \(G_q(s_i)\in\mathbb N_0\), logo o numerador é racional e,
dividido por \(\lvert I\rvert\in\mathbb Z_{>0}\), o score é racional. Para a
convergência, \(\lfloor\alpha n\rfloor\le\alpha n<\lfloor\alpha n\rfloor+1\)
implica
\[
\left\lvert\frac{\lfloor\alpha n\rfloor}{n}-\alpha\right\rvert
=\frac{\alpha n-\lfloor\alpha n\rfloor}{n}<\frac1n,
\]
e portanto \(\lfloor\alpha n\rfloor/n\to\alpha\). Cada \(G_{q,n}\) é um campo de
contagem legítimo (conserva \(n=\lvert I_n\rvert\), isto é,
\(G_{q,n}(0)+G_{q,n}(1)=n\); Prop.~\ref{prop:gq-conserva}) e cada score
\(\lfloor\alpha n\rfloor/n\) é um quociente de inteiros, logo racional. Por
último, \(\alpha=\sqrt2/2\notin\mathbb Q\) pela irracionalidade clássica de
\(\sqrt2\).
\end{proof}

\begin{observacao}[O irracional é do limite, não da realização]
\label{obs:score-limite}
A irracionalidade em Prop.~\ref{prop:score-limite} pertence ao
\emph{valor-limite} da sequência, não a qualquer realização finita individual:
nenhum \(\mathbb E_{G_n}[S]\) é irracional. A cadeia conceptual é
\[
\boxed{
\text{inteiro}
\;\longrightarrow\;
...
\]
e o último passo não exige representar um irracional dentro do motor finito —
basta o par \((N,D)\) da Def.~\ref{def:esperado} com \(N=\sum_i s_iG_q(s_i)\)
e \(D=\lvert I\rvert\); a divisão racional fica fora. É uma propriedade
matemática da construção de campos de contagem (realização \(\to\) projeção
\(\to\) score); não se atribui o limite ao JEV nem à sua implementação interna:
\[
\boxed{
\text{realização finita}
\;\neq\;
\text{sequência/limite}
\;\neq\;
\text{implementação interna do JEV}.
}
\]
\end{observacao}
...
\]
Seja \(\Delta(Y)\) o simplex das distribuições de probabilidade sobre o finito
\(Y\). O funcional de leitura \(L\) especializa-se, em cada caso com
contradomínio e forma próprios sobre \(p_q\in\Delta(Y)\):
\begin{itemize}[leftmargin=1.5em]
\item \(Y=\{0,1\}\) --- Noul:
      \[
      L\colon\Delta(\{0,1\})\to[0,1],\qquad L(p_q)=p_q(1);
      \]
\item \(Y=C\) categorial --- Choice:
      \[
      L\colon\Delta(C)\to C\times[0,1],\qquad
      L(p_q)=\bigl(c^\star,\;\kappa(p_q)\bigr),\quad c^\star\in\arg\max p_q;
      \]
\item \(Y=S\) ordenado --- Score:
      \[
      L\colon\Delta(S)\to\mathbb R,\qquad
      L(p_q)=\mathbb E_G[S]=\sum_i s_i\,p_q(s_i),
      \]
      com \(s_i\in\mathbb R\) e \(s_0<\dots<s_m\); alternativa discreta:
      \(c^\star=\arg\max_i p_q(s_i)\).
\end{itemize}
\end{proposicao}
...
% ═══════════════════════════════════════════════════════════════════════════
\section{Validação experimental da construção}

A construção proposta foi levada ao motor wasm da Tiffany no piloto
\texttt{JEV-WASM} (experiências E1--E6, fechadas; registo em
\texttt{docs/INTEGRACAO\_JEV\_WASM.md}). Preserva-se a distinção:
\[
\boxed{
\text{contrato JEV documentado}
\neq
\text{implementação interna JEV}
\neq
\text{realização experimental Tiffany}.
}
\]
O módulo de contagem
(\texttt{conecthus/backends/wasm/jev/marginal.c}
\(\to\) \texttt{assets/figuras/wasm/jev/marginal.wasm}) re-conta o campo \(G\)
sobre realizações determinísticas (LCG fixo), e cada saída é comparada com um
oráculo em JS puro --- o selo
\(\text{WASM}\equiv\text{oráculo}\equiv\text{esperado}\). A cadeia experimental
validada é
\[
\boxed{
G
...
\]
fechada na prática do motor: do campo à projecção \(q\), ao par racional exacto
\(N=\sum_i s_iG_q(s_i)\), \(D=\lvert I\rvert\)
(Prop.~\ref{prop:score-limite}, Def.~\ref{def:esperado}), e à média \(N/D\)
\emph{lida do par}, sem divisão no motor.

Resultados fechados:
\[
\text{E1--E5: }509/509,\qquad
\text{E6: }68/68,
\]
\[
\boxed{577/577\text{ verificações, }0\text{ falhas}.}
\]

Cada bloco valida:
\begin{itemize}[leftmargin=1.5em]
\item \textbf{E1} --- conservação: \(\sum_x G(x)=\lvert I\rvert\)
      (Prop.~\ref{prop:conservacao});
\item \textbf{E2} --- marginalização: \(G_{q_i}\) recuperado por somas do campo
      conjunto, exacto em inteiros (Teor.~\ref{thm:marginal});
\item \textbf{E3} --- múltiplas leituras sobre a mesma realização:
      Noul/Choice/Score como marginais de uma única leitura conjunta
      (Cor.~\ref{cor:marginais}, Obs.~\ref{obs:multi-q-marg});
\item \textbf{E4} --- Score e par exacto \((N,D)\): \(\mathbb E_G[S]=N/D\) com
      \(N=\sum_i s_iG_q(s_i)\), sem divisão no motor
      (Prop.~\ref{prop:score-limite});
\item \textbf{E5} --- leituras paralelas sobre o mesmo campo: Noul e Score no
      mesmo percurso, campo \(G\) não recriado;
\item \textbf{E6} --- integração no front: demonstração ao vivo do selo na
      página da aplicação --- integração, não prova nova.
\end{itemize}

A formulação de leitura é: os experimentos \emph{validam a realização
computacional da construção proposta e a sua compatibilidade com os contratos
de saída considerados}; eles não identificam nem reconstroem o mecanismo
interno do JEV. Os testes cobrem casos testados; os teoremas continuam a valer
pelas suas demonstrações. Artefactos versionados:
\code{tests/jev\_backends.js}, \code{tests/jev\_front.js},
\code{conecthus/backends/wasm/jev/marginal.c},
\code{assets/figuras/wasm/jev/marginal.wasm} e
\code{app/src/jev\_contratos.js}. Os invariantes gerais que esta secção instancia
estão registados e caracterizados em \texttt{campos.tex} (secção de validação
computacional); os dois textos referem-se mutuamente e não duplicam provas.
Esta é a instanciação \emph{tipada}; a evidência reforça a construção, não a
transforma em implementação do JEV.
...
Validação experimental (piloto E1--E6) & --- & 577/577 verificações, 0 falhas; WASM $\equiv$ oráculo sobre casos testados & validação de implementação (não prova o mecanismo) \\
\bottomrule
\end{tabular}
\end{center}
