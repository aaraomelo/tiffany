# captura — Descrição Matemática e Script GLSL

## Card
`captura` — 6s, 24fps, 640×640 (gravado em `gifs/captura_6s_24fps.gif`)

## Equação aplicada

$$objeto → poinsética → 2ª garrafa → reconstrói$$
...
O GLSL é gerado dinamicamente por `cards_kernel.js` → `cards_campo.js`. O kernel do captura aplica:

```glsl
// captura: objeto → poinsética → 2ª garrafa → reconstrói
...
- gifs/captura_6s_24fps.gif — GIF gravado
