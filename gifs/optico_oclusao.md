# optico_oclusao — Descrição Matemática e Script GLSL

## Card
`optico_oclusao` — 6s, 24fps, 640×640 (gravado em `gifs/optico_oclusao_6s_24fps.gif`)

## Equação aplicada

$$ao = 1 − Σ (h − sdf(p+n·h))·scaⁱ$$
...
O GLSL é gerado dinamicamente por `cards_kernel.js` → `cards_campo.js`. O kernel do optico_oclusao aplica:

```glsl
// optico_oclusao: ao = 1 − Σ (h − sdf(p+n·h))·scaⁱ
...
- gifs/optico_oclusao_6s_24fps.gif — GIF gravado
