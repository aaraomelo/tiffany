# tex_yasmin — Descrição Matemática e Script GLSL

## Card
`tex_yasmin` — 6s, 24fps, 640×640 (gravado em `gifs/tex_yasmin_6s_24fps.gif`)

## Equação aplicada

$$cos 5(θ−γ) · as 5 pontas nas 5 pétalas$$
...
O GLSL é gerado dinamicamente por `cards_kernel.js` → `cards_campo.js`. O kernel do tex_yasmin aplica:

```glsl
// tex_yasmin: cos 5(θ−γ) · as 5 pontas nas 5 pétalas
...
- gifs/tex_yasmin_6s_24fps.gif — GIF gravado
