# coracao_revela — Descrição Matemática e Script GLSL

## Card
`coracao_revela` — 6s, 24fps, 640×640 (gravado em `gifs/coracao_revela_6s_24fps.gif`)

## Equação aplicada

$$h = g∘f⁻¹ · esfera → coração, erro 0$$
...
O GLSL é gerado dinamicamente por `cards_kernel.js` → `cards_campo.js`. O kernel do coracao_revela aplica:

```glsl
// coracao_revela: h = g∘f⁻¹ · esfera → coração, erro 0
...
- gifs/coracao_revela_6s_24fps.gif — GIF gravado
