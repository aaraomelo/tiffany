# tex_merlin — Descrição Matemática e Script GLSL

## Card
`tex_merlin` — 6s, 24fps, 640×640 (gravado em `gifs/tex_merlin_6s_24fps.gif`)

## Equação aplicada

$$cos 1(θ−γ) · as 1 pontas nas 1 pétalas$$
...
O GLSL é gerado dinamicamente por `cards_kernel.js` → `cards_campo.js`. O kernel do tex_merlin aplica:

```glsl
// tex_merlin: cos 1(θ−γ) · as 1 pontas nas 1 pétalas
...
- gifs/tex_merlin_6s_24fps.gif — GIF gravado
