# optico_sombra — Descrição Matemática e Script GLSL

## Card
`optico_sombra` — 6s, 24fps, 640×640 (gravado em `gifs/optico_sombra_6s_24fps.gif`)

## Equação aplicada

$$res = min(1, k·sdf(p+L·t)/t) · marcha até a luz$$
...
O GLSL é gerado dinamicamente por `cards_kernel.js` → `cards_campo.js`. O kernel do optico_sombra aplica:

```glsl
// optico_sombra: res = min(1, k·sdf(p+L·t)/t) · marcha até a luz
...
- gifs/optico_sombra_6s_24fps.gif — GIF gravado
