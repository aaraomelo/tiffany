# optico_especular — Descrição Matemática e Script GLSL

## Card
`optico_especular` — 6s, 24fps, 640×640 (gravado em `gifs/optico_especular_6s_24fps.gif`)

## Equação aplicada

$$spec = max(n·H, 0)^s · H = (L+V)/|L+V|$$
...
O GLSL é gerado dinamicamente por `cards_kernel.js` → `cards_campo.js`. O kernel do optico_especular aplica:

```glsl
// optico_especular: spec = max(n·H, 0)^s · H = (L+V)/|L+V|
...
- gifs/optico_especular_6s_24fps.gif — GIF gravado
