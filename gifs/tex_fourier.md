# tex_fourier — Descrição Matemática e Script GLSL

## Card
`tex_fourier` — 6s, 24fps, 640×640 (gravado em `gifs/tex_fourier_6s_24fps.gif`)

## Equação aplicada

$$cos 8(θ−γ) · as 8 pontas nas 8 pétalas$$
...
O GLSL é gerado dinamicamente por `cards_kernel.js` → `cards_campo.js`. O kernel do tex_fourier aplica:

```glsl
// tex_fourier: cos 8(θ−γ) · as 8 pontas nas 8 pétalas
...
- gifs/tex_fourier_6s_24fps.gif — GIF gravado
