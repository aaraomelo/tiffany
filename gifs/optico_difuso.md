# optico_difuso — Descrição Matemática e Script GLSL

## Card
`optico_difuso` — 6s, 24fps, 640×640 (gravado em `gifs/optico_difuso_6s_24fps.gif`)

## Equação aplicada

$$I = max(n·L, 0) · f\_r = ρ/π · isotrópico$$
...
O GLSL é gerado dinamicamente por `cards_kernel.js` → `cards_campo.js`. O kernel do optico_difuso aplica:

```glsl
// optico_difuso: I = max(n·L, 0) · f_r = ρ/π · isotrópico
...
- gifs/optico_difuso_6s_24fps.gif — GIF gravado
