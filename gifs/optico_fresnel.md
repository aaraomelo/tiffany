# optico_fresnel — Descrição Matemática e Script GLSL

## Card
`optico_fresnel` — 6s, 24fps, 640×640 (gravado em `gifs/optico_fresnel_6s_24fps.gif`)

## Equação aplicada

$$F = F₀ + (1−F₀)(1−cos θ)⁵ · rim = (1−n·V)^p$$
...
O GLSL é gerado dinamicamente por `cards_kernel.js` → `cards_campo.js`. O kernel do optico_fresnel aplica:

```glsl
// optico_fresnel: F = F₀ + (1−F₀)(1−cos θ)⁵ · rim = (1−n·V)^p
...
- gifs/optico_fresnel_6s_24fps.gif — GIF gravado
