...
| `agm_gerador.c`| **a dinâmica do AGM É a do gerador**: conjugação explícita `h : k ↦ τ=K′/K` leva o AGM em `τ↦2τ`, e a torre da torção é `w_d=(w_{2d})²` — o índice dobra. **Mesmo mapa, duas leituras.** No expoente cabe numa linha e é exato em `ℤ_p`: `2·log√(ab) ≡ log a + log b` (261/261) — `⊕` é a média aritmética no **valor**, `⊗` a mesma média no **expoente** |
