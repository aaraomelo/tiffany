...
zero, o que equivale a a própria medição ter $\lambda_{\text{med}}=0$ --- não amplifica o erro da
leitura. Uma medição que \emph{perde} um bit não tem inversa, o resíduo é $>0$, e por
$\S\ref{sec:central}$ (o estado é o canal do caos) ela é ela própria caótica.
\end{enumerate}
\end{teorema}
...
E $T^{-1}\circ T=\mathrm{id}$ com resíduo $0$ diz que nada se acumula entre uma leitura e a seguinte,
logo $\lambda_{\text{med}}=0$. Um passo com perda --- um bit descartado --- não é injectivo: não há
$T^{-1}$ que o desfaça, o resíduo da volta é $>0$, e esse resíduo entra na recorrência da leitura
seguinte e amplifica ($\lambda_{\text{med}}>0$). A volta é a medida; sem ela não há medida, há deriva.
\end{proof}

\noindent \textbf{E é esta a justificativa das proibições.} O \code{double} \emph{arredonda} ---
descarta um bit em cada operação, e não há volta: mede com $\lambda_{\text{med}}>0$. O \code{malloc} e
