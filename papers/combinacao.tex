...
%  Continuação de papers/redes.tex (§RG6 FECHADO); não altera redes.tex.
...
\code{tests/redes\_combinacao.js} (174 asserts);
...
\text{bilateral, bem definido, }\tau,\,G,\,\zeta/\mu\text{-compatível}\bigr\}.
\]
...
L5 invariância $\tau$ (realização) & $\checkmark$ & $\checkmark$ \\
...
\noindent\textbf{Aberto --- após C0--L7.}
\begin{enumerate}\itemsep3pt
...
\]
de operações observáveis bilateralmente dependentes, bem definidas, e compatíveis com
$\tau$, $G$ e $\zeta/\mu$. Dentro dela há pelo menos duas geometrias (lexMax, rectCell).
...
\medido{\textbf{Bateria C0--L7} (\code{node tests/redes\_combinacao.js}):\\
174 asserts, 0 falhas; \code{lib/arena\_combinacao.mjs};\\
integrada em \code{tools/auditoria\_rg6.bat} após §RG6.\\
\code{redes.tex} permanece congelado como documento de interface.\\
