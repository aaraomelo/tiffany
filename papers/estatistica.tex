...
A ligação com a transformada e com a assinatura do corpo fica aqui explícita. O dado primário
não é a distribuição; é a \textbf{identidade discreta} do sistema, isto é, o pacote de invariantes
que fixa a classe do corpo e a sua leitura em qualquer representação compatível:
\[
\operatorname{Sig}(B)=\bigl(N,\operatorname{tr},\Delta,p,q,r,\tau,\ldots\bigr).
\]
Quando essa assinatura é completa e fiel para a classe considerada, vale
\[
\operatorname{Sig}(B_1)=\operatorname{Sig}(B_2)
\iff
B_1\sim B_2.
\]
Em outras palavras, o objeto não é reconstruído por Born; é reconstruído pela identidade discreta
que os invariantes preservam. A estatística só aparece depois do invariante, na forma de
normalização do peso já contado pela estrutura. Em termos de dívida técnica, esta correção
paga a questão central: a probabilidade não é a origem da estrutura; ela é a leitura normalizada
de uma multiplicidade que já foi determinada pela identidade discreta do corpo.
