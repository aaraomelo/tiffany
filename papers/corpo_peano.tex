...
% O corpo operacional de Peano --- eixo: Teorema do Maestro = realização dinâmica do central.
% Cadeia: borda → ordem → massa → contínuo → torre → retração π_k → música.
...
\gkcapa{O Corpo de Peano}
       {construção até à retração --- o Teorema do Maestro}
       {realização dinâmica do teorema central; o tradutor \texttt{.tex}$\leftrightarrow$PDF opera nesta torre}
\maketitle
...
entre andares}. O \textbf{Teorema do Maestro}
(Teorema~\ref{thm:proj-maestro}) é essa realização ---
$\mathcal{P}_k=\mathrm{tick}\circ\mathrm{batuta}\circ\Pi$ executa
$\pi_k$ a cada tick. A música não é metáfora decorativa: é a
...
\code{tests/corpo\_peano.c} §CP1--§CP39.
\end{abstract}
...
\text{retração }\pi_k
\;\Longrightarrow\;
\text{Teorema do Maestro}
\;\Longrightarrow\;
\text{realização musical}.
}
\]
A construção (§§ iniciais / contínuo / torre) chega à retração; o
Teorema do Maestro (\S\ref{sec:maestro-thm}) é o eixo; a música
...
Gentil--Hurwitz (central) & citação / eixo (destino: maestro) \\
