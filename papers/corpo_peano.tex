...
espaço & Morfologia (Lei~4) & dilata / contrai o suporte \\
...
\begin{teorema}[Morfológico --- realização espacial da Lei~4]\label{thm:morfologico}
A Lei~4 (dobra, $T+T^{*}$, $d\mapsto 2d$, interfaces $\Delta=m^{2}+4$)
é o fundamento algébrico da \textbf{contração/dilatação}. O par
Maestro\,/\,Metrónomo realiza essa dualidade no \emph{tempo}
(Teoremas~\ref{thm:proj-maestro},~\ref{thm:metronomo}). O
\textbf{corpo morfíco} realiza a \emph{mesma} dualidade no
\textbf{espaço} --- sem nova lei, sem Lei~$9$:
\[
\boxed{
(\mathrm{Maestro},\,\mathrm{Metr\acute{o}nomo})
\;\longleftrightarrow\;
(\text{dilata\c{c}\~{a}o},\,\text{contra\c{c}\~{a}o}).
}
\]
Seja $R_k$ o suporte disponível no andar $k$ (a página como dimensão
do relógio, $\pi_{\mathrm{p\acute{a}gina}}$). A morfologia
\[
\boxed{
\mathcal{M}_k\colon A_k\longrightarrow R_k,
\qquad
\mathcal{M}_k=\mathcal{D}_k\circ\mathcal{C}_k^{-1}
}
\]
adapta o suporte espacial preservando a assinatura:
\[
\boxed{
\mathcal{M}_k(\Pi)\sim\Pi
}
\]
--- mesma realização estrutural (duração, ordem dos ticks, vozes,
métrica, linha dourada), não a mesma coordenada gráfica. Ordem da
torre no suporte:
\[
\boxed{
\text{largura}\;\longrightarrow\;\text{altura}
}
\]
primeiro preenche $[0,W]$ (eixo do Metrónomo); depois ajusta $[0,H]$.
\[
\boxed{
\begin{array}{c@{\quad}c}
\text{dilata\c{c}\~{a}o} & \text{ocupa o suporte} \\
\text{contra\c{c}\~{a}o} & \text{reencontra o suporte} \\
\hline
\text{dilata\c{c}\~{a}o}+\text{contra\c{c}\~{a}o} & \text{mesma realiza\c{c}\~{a}o}.
\end{array}
}
\]
\[
\boxed{
\underbrace{\Pi}_{\text{conte\'{u}do}}
\qquad
\underbrace{\mathcal{M}}_{\text{suporte}}.
}
\]
\emph{Prova.} (1)~Lei~4 já contém a passagem entre escalas/suportes
e $\Delta$ nas interfaces. (2)~Maestro projecta no tempo; Metrónomo
lê a volta ($\lambda^{+}+\lambda^{-}=0$). (3)~$\mathcal{D}$/$\mathcal{C}$
são o dual espacial dessa projecção/leitura --- a página deixa de ser
recipiente e passa a participação do suporte na realização. Não se
infla o catálogo: oito leis; morfologia $=$ realização espacial da
dobra.
\end{teorema}

\begin{center}\small
\begin{tabular}{@{}lll@{}}
temporal & espacial & objecto \\
\hline
Maestro & dilatação $\mathcal{D}$ & projecta / ocupa \\
Metrónomo & contração $\mathcal{C}$ & lê / reencontra \\
tick & coordenada & marca \\
relógio & página & suporte \\
$\lambda^{+}$ & expansão & frente \\
$\lambda^{-}$ & contração & volta \\
volta fechada & fechamento morfíco & $\mathcal{M}(\Pi)\sim\Pi$ \\
\end{tabular}
\end{center}
...
A figura~\ref{fig:partitura-sistema} resume as vozes; a realização desenhada
está em \code{papers/partitura.tex} (Peano: $\nu$, trial, $\pi_k$, leis,
naipes) e a tipográfica em \code{partitura/render/}.
\end{observacao}
...
4 & $T+T^{*}$ & tecidos & $d\mapsto2d$ & dobra & morfologia $\mathcal{M}$ (larg.$\!\to$alt.) \\
