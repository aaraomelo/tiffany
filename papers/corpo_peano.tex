...
%----------------------------------------------------------------------
\subsection*{Interpretação dinâmica --- metrónomo, maestro, orquestra}

\begin{observacao}[estrutura de projecção do maestro]\label{obs:proj-dinamica}
A tríade opera como transmissão entre níveis; o operador que desce é
$\pi_k$ (Def.~\ref{def:pi-k}), realizado pelo maestro
(Teorema~\ref{thm:proj-maestro}) --- não uma lei nova:
\[
\boxed{
\mathrm{Metr\acute{o}nomo}
\;\longrightarrow\;
...
\]
Papéis: metrónomo $=$ \emph{quando}; batuta $=$ \emph{como}
(pera + vareta + ponta); partitura $\Pi=$ \emph{o quê}; orquestra $=$
\emph{onde}; conservatório $=$ \emph{acima}. O maestro junta os quatro
da realização; a pera acopla o cone do conservatório ao canal $\pi_k$.
\end{observacao}
...
\]
Em dimensão: $d_{k+1}/2=d_k$. No catálogo de fases (leis), a mesma
forma desce uma $-$alidade por passo (hexal$\to\cdots\to$dual;
\code{tests/milenio\_trial.c} §M8). No cone: a sombra no hiperplano é
$\pi$ geométrico (\code{tests/sombra\_cone.c}). Estatuto:
\emph{definição} --- já na torre; não é objecto novo do maestro.
\end{definicao}

\begin{teorema}[princípio de projecção do maestro]\label{thm:proj-maestro}
Três afirmações distintas --- não se confundem:
\begin{enumerate}
\item \textbf{Retração} (álgebra da torre). $\pi_k$ (Def.~\ref{def:pi-k})
satisfaz $\pi_k\circ\iota_k=\mathrm{id}_{A_k}$ e $d_{k+1}/2=d_k$.
Isto é propriedade dos andares; o maestro não a cria.
\item \textbf{Conservação} (classe Gentil). Se o passo é da classe com
cruz/norma conservada ($T+T^{*}$), então
$|X_{k+1}|_{\mathrm{Gentil}}=|X_k|_{\mathrm{musical}}$.
Não segue só da retração: é hipótese de classe.
\item \textbf{Realização musical} (interpretação dinâmica). Com
metrónomo (tick $t$), batuta (pera\,/\,vareta\,/\,ponta) e carga
$\Pi$, o maestro \emph{transporta} $\pi_k$:
\[
\boxed{
\mathcal{P}_k
\;=\;
\pi_k
\quad\text{(canal da batuta; não é operador novo)}.
}
\]
\end{enumerate}
\[
...
\mathcal{M},
\]
e $\Pi=(d,S,G,\ell,C,\Delta,\ldots)$ selecciona a fibra admissível.
\[
\boxed{
\text{O maestro não compõe o corpo: projecta-o.}
}
\]
A música é a \textbf{sombra dinâmica da retração}. O cone do
conservatório assenta na pera; a órbita abaixo escreve-se na ponta.
\emph{Prova.} (1)~Def.~\ref{def:pi-k}. (2)~cruz do passo de tecidos
(\emph{Corpo estelar}; Prop.~\ref{thm:central-peano} no contínuo),
quando a classe o admite. (3)~a batuta e a orquestra são transporte
de $\pi_k$, não fonte: $\mathcal{P}_k$ não acrescenta operador à torre.
\end{teorema}
...
retração $\neq$ conservação $\neq$ realização;
$\mathcal{P}_k=\pi_k$; maestro projecta, não compõe.
