...
realização transporta uma \emph{medida}, e há transformações que a conservam
exatamente --- as de determinante $\pm1$. O que elas não conservam é o percurso.
...
espiral quadrada do mesmo comprimento têm ambas $\abs{I}=2^{12}+1$ e
$\sum G=\abs{I}$, e multiplicidades máximas distintas.
\end{proof}
...
reconta exatamente o que a métrica esquece, e por isso a informação tem de ficar
...
\emph{vizinhança}, e é o passo~2 do ciclo. É aqui, e só aqui, que a estrutura de
$X$ é chamada; e é aqui que o passo~3, o gradiente, vai buscar o mínimo.
...
distintos. Além disso, as cláusulas (1)--(3) do
...
representação sem auto-interseção no espaço total. A reta injetiva é o caso
...
\noindent (Pelo Teor.~\ref{thm:shift}, $\zeta=(1-S)^{-1}$: a convolução com
$\zeta$ é a soma de todas as potências do deslocamento.)
...
Para $k,j\in(\Z/2)^m$ escreva-se $\langle k,j\rangle$ a paridade do número de
...
\medskip\noindent Daqui sai a leitura da transformada do campo: pelo
Teor.~\ref{thm:dirac}, $F(G)_k=\sum_t\chi_k(\pi(t))$ é o \textbf{saldo} da
trajetória no corte $k$ --- de que lado dele esteve, somado sobre os passos. Os
caracteres não foram escolhidos: são a diagonalização deste grafo.
...
Seja $P=\bigl\lvert\{(i,j): i<j,\ \pi(i)=\pi(j)\}\bigr\rvert$ o número de pares
dobrados. Então
\[
...
Seja $d(t)=[\,G_{t}(\pi(t))>1\,]$ --- vale $1$ quando o salto chega a uma célula
que já tinha marca --- e $D=d*\zeta$ a sua acumulação, a \textbf{taxa de dobras}
do percurso. O \textbf{tempo} é a variação dessa taxa ao saltar de um vértice
para o vizinho, indexada pelo parâmetro: $\mu D=d$.
\end{definicao}
...
\begin{teorema}[$\zeta$ e $\mu$ saem de $S$]\label{thm:shift}
$\zeta=\sum_{j\ge0}S^{j}=(1-S)^{-1}$ e $\mu=1-S$.
\end{teorema}

\begin{proof}
$(\sum_{j\ge0}S^j a)(t)=\sum_{j\ge0}a(t-j)=\sum_{u\le t}a(u)=(a*\zeta)(t)$, com
a soma finita porque $a$ se anula abaixo de $0$. E $(1-S)$ aplicado a isso
telescopa: $(a*\zeta)(t)-(a*\zeta)(t-1)=a(t)$. Logo $(1-S)\zeta=\mathrm{id}$, e
$\mu=1-S$ pelo Teor.~\ref{thm:mu}.
\end{proof}
...
Heighway e a reta axial são percursos; a reta \emph{diagonal} não é --- o seu
passo $(1,1)$ não é aresta ---, e a rotação $J$ também não, porque salta.
...
nada no ciclo distingue este caso dos outros \\
...
Def.~\ref{def:tempo} §AN20 ($\mu D=d$ com resíduo $0$; e a mesma trajetória com
...
Teor.~\ref{thm:leitura} (dobrar sem período obriga a ler o índice)}
        {campo esparso, $\Theta(n)$/passo, memória $\Theta(|I|)$, sem alocação dinâmica;
         $\Z^1$--$\Z^4$, árvore, $\Z[i]$}
        {\code{tests/aranha\_n.c} §AN1--§AN22;
