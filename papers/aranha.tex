...
\medskip\noindent É a mesma dobra $T_{k+1}=T_k+T_k^{*}$ da §\ref{sec:dragao}: o
grafo em que a transformada da §\ref{sec:transf} vive é construído pelo passo que
constrói a curva.
...
% ═══════════════════════════════════════════════════════════════════════════
\section{O que o passo lê: a célula ou o índice}\label{sec:leitura}
...
\midrule
a marca fica & no contradomínio $X$: é $G$ & no domínio $I$: é o índice \\
...
\medskip\noindent\textbf{E verifica-se.} Que o passo lê o índice não se
...
Teor.~\ref{thm:leitura} (dobrar sem período obriga a ler o índice)}
        {campo esparso, $\Theta(n)$/passo, memória $\Theta(|I|)$, sem alocação dinâmica;
         $\Z^1$--$\Z^4$, árvore, $\Z[i]$}
        {\code{tests/aranha\_n.c} §AN1--§AN18;
