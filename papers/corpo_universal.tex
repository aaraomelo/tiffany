...
6 & \textbf{Inversor total} & $R_{\mathrm{total}}=(R_{\mathrm{end}},R_E,
  R_\Phi,R_{\Phi_2},R_D)$: RETAIN $\iff$ tudo zera & \code{residuos\_totais.js}
  $9$:$0$ \\
\bottomrule
\end{tabular}
\end{center}
...
\quad$\hookrightarrow$ Clifford & $D^{2}=L$ (a assinatura necessária) &
  --- & \code{dirac\_transicao.js} \S D6 & parcial \\
\bottomrule
\end{tabular}
\end{center}
...
dualidade dos caracteres); candidato natural para o limite da escada de
observadores (discreto$\to$contínuo).
