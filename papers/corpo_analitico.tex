...
\newtheorem{lei}{Lei}
% ── OS AMBIENTES QUE ESTE DOCUMENTO USA E NÃO DECLARAVA ─────────────────────
% `observacao` e `proposicao` aparecem no corpo do texto e não estavam
% definidos: um `\begin{...}` sem `\newtheorem` é erro de compilação, não um
% estilo por acertar. É a mesma doença da macro por definir, um andar ao lado —
% e do mesmo tipo que o `\ref` órfão: o defeito está escrito e daqui não se vê.
\newtheorem{proposicao}[teorema]{Proposição}
\newtheorem{observacao}[teorema]{Observação}
\renewcommand{\proofname}{Demonstração}
\newcommand{\dg}{^{\dagger}}
% ── AS MACROS DA CASA QUE ESTE DOCUMENTO USA E NÃO DEFINIA ──────────────────
% `\abs` e `\colunas` são usadas no corpo do texto e viviam só nos irmãos. Este
% documento é um `article` independente — não herda preâmbulo de ninguém —, pelo
% que um comando por definir aqui é um erro de compilação, não um estilo por
% acertar. É a mesma doença da referência órfã, um andar acima: o defeito está
% escrito e só aparece em quem compilar.
...
% ── REFERÊNCIAS PARA O `corpo_algebrico` ────────────────────────────────────
% Este documento cita 19 enunciados do irmão algébrico, e uma \ref que atravessa
% documentos NÃO falha o pdflatex: emite Warning e imprime «??» no PDF. O `xr`
% lê o .aux do outro e resolve-as.
%
% E com PREFIXO, que não é decoração: `sec:torre` existe nos DOIS documentos, e
% sem prefixo o LaTeX diria «multiply defined» e um dos dois ganharia em
% silêncio. Com `alg:` as duas famílias ficam separadas por construção — e o
% leitor vê no próprio texto que a referência é externa.
%
% Requer que `corpo_algebrico.aux` exista: compilar o algébrico primeiro.
\usepackage{xr}
\externaldocument[alg:]{corpo_algebrico}
...
\noindent\textbf{Primeiro, as duas caixas, que são duas.} O \S\ref{alg:sec:corte}
...
\S\ref{alg:sec:pontofixo} já disse \emph{o que} é esse ponto: o \textbf{ponto fixo da
...
$\abs{\sigma\dg}<1$ (\S\ref{alg:sec:pisot}). \textbf{A completude não é aqui um
...
\S\ref{alg:sec:corte}, que é o caso metálico:
\[
...
(Teor.~\ref{alg:thm:encaixe}). A cadeia é
...
o \textbf{swap} da Def.~\ref{alg:def:lei0},
\[
...
$p^{2}=2q^{2}$ é a descida proibida (Cor.~\ref{alg:cor:habitante}). $\square$
\end{proposicao}
...
por construção (\S\ref{alg:sec:dual}). A continuidade clássica precisa do limite
...
onde as folhas colapsam (Cor.~\ref{alg:cor:dourada-completa}).
\end{definicao}
...
\S\S\ref{alg:sec:dual}--\ref{sec:todos}.
...
essa é a do \S\ref{alg:sec:operacao}, $L_0,\ldots,L_7\to\star\to\tau\to E_{+}\oplus E_{-}\to
...
\begin{teorema}[contar pelo domínio e medir por níveis somam ao rectângulo]\label{thm:rectangulo}
\[
\boxed{
\begin{array}{ll}
...
\textbf{é a base que as carrega}, e é ela que a base ortonormal do \S\ref{alg:sec:orto}
realiza.
...
do zero} do Teor.~\ref{alg:thm:divzero}: lá o polinómio a factorizar, aqui a álgebra a
...
$\mathbb{N}$ não há descida infinita. \emph{É a mesma descida do Teor.~\ref{alg:thm:corte-fixo},
...
Def.~\ref{alg:def:espaco} põe-nas: \textbf{(D1)} $\alpha=\alpha^{\ast}\iff\alpha\in\mathbb{Q}$
...
Há \textbf{uma} transformada, e o Teor.~\ref{alg:thm:espectro} já a nomeou: \textbf{avaliação
...
Cor.~\ref{alg:cor:conv-desce} o produto é a convolução dos coeficientes seguida da redução ---
...
Teor.~\ref{alg:thm:espectro} lido neste corpo, e é a razão de a transformada existir.

\textbf{(2) A VOLTA é a DECONVOLUÇÃO, e é a do Teor.~\ref{alg:thm:decon-andar}.} Dividir na
...
o corpo parte-se, e aparecem os divisores de zero do Teor.~\ref{alg:thm:divzero}, não nulos sem
...
$a^{2}-b^{2}m^{2}D$. São as duas do Teor.~\ref{alg:thm:fixo-dual}, agora com a razão à vista:
...
diagonalização é o Teor.~\ref{alg:thm:espectro}, a volta é o Teor.~\ref{alg:thm:decon-andar}, a
série é o Teor.~\ref{thm:serie-quadratica}, o par traço/norma é o Teor.~\ref{alg:thm:fixo-dual}.
...
O Teor.~\ref{alg:thm:espectro} diz \emph{«não em raízes da unidade»}, e no anel da Lei~8 a
...
\end{center}
\emph{A cláusula do Teor.~\ref{alg:thm:espectro} lê-se então sem excepção}: as folhas são
...
informação perde-se. É o Teor.~\ref{alg:thm:decon-andar} dito na transformada. $\square$
\end{corolario}
...
folhas (Teor.~\ref{alg:thm:fixo-dual}), logo
\[
\operatorname{Cruz}=0
...
\emph{A raiz disto é a propriedade} \textbf{(P1)} \emph{da Def.~\ref{alg:def:espaco}}: só os
...
\textbf{corta-o}, e o corte que produz é o do Teor.~\ref{alg:thm:corte}. As duas coisas são
uma.
...
(Teor.~\ref{alg:thm:fixo-dual}) --- são \textbf{ambas inteiras}:
\[
...
\]
--- que é, letra por letra, a partição em dois casos do Teor.~\ref{alg:thm:corte}. \emph{A raiz
...
Teor.~\ref{alg:thm:descida} ($x=x\dg\iff x\in\mathbb{Q}$) dito na transformada. E daqui sai a
leitura que faltava:
\begin{center}
...
racional cai em cima do corte}, que é a condição de corte do Teor.~\ref{alg:thm:corte},
...
Teor.~\ref{alg:thm:descida}: $\mathbb{Q}$ são as classes dos pontos fixos sob o traço;
