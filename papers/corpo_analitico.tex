...
\]
\emph{A seta aponta do centro para cá.} O Universal é o \textbf{ponto fixo} do trial --- o
...
\smallskip
Do \emph{Corpo Universal} recebe o \textbf{operador} e a \textbf{medida}; o que
...
\text{a Lei } 1\dg=-1 & \code{universal thm:fixo-dual} & \text{a leitura: o que separa}\\[2pt]
\text{a Lei } T\dg=-T,\ J & \code{universal thm:espectro-tau} & \text{o passo: o que roda}\\[2pt]
\text{a operação }\star & \code{universal thm:operacao} & \text{a soma e o produto do corpo}\\[2pt]
\text{a Lei 0},\ 0\dg=\infty & \code{universal def:lei0} & \text{a divisão do zero}\\[2pt]
\text{os três sinais }\tau & \code{universal thm:cuspide} & \text{os três regimes da estrela}\\[2pt]
\text{a torre } \dim A_n=2^n & \code{universal thm:torre} & \text{os andares do corpo}\\[2pt]
\text{a conservação }\abs{\det}=1 & \code{universal thm:conservacao} & \text{a exigência, escrita}\\[2pt]
\text{o espaço } \mathbb{Q}(m\sqrt D) & \code{universal def:espaco} & \text{a base de tudo}\\[6pt]
...
existe no \code{universal.tex}; \code{tests/cuspide.c} \S C12 mede que $\tau=+1$ é o
...
(\code{universal thm:conservacao}), escrita na língua desta camada. \emph{Não se funda
...
(\code{universal thm:fixo-dual}); a segunda é o espectro da involução
(\code{universal thm:espectro-tau}), de onde sai o $J$ de período quatro. \emph{As provas
...
(\code{universal def:lei0}, \code{universal thm:lei0}), com a factorização do zero em
\code{universal thm:divzero}. \emph{Aqui} ela deixa de ser uma troca de coordenadas e passa
...
duas metades (\code{universal thm:operacao}). \emph{Aqui} elas aparecem como soma e produto
...
cúspide (\code{universal thm:cuspide}) --- o mesmo trial que divide a tríade. \emph{Aqui}
...
\noindent\emph{Vem do centro:} $\dim A_n=2^n\dim A_0$ (\code{universal thm:torre}).
...
mura na $8$ --- e o \textbf{limite dimensional é função da luz da dimensão}: ela fecha quando o erro
do seu $\pi$ já não move meia marca de $c_n$, e a primeira que fecha é $n=4$. Daí a regra do
...
não nas ímpares; na torre $n=2,4,8,16$ o $q_n$ é potência de $2$ pura; $\pi_n$ sai da meia-corda de
Arquimedes com erro a cair dois bits por dobra (a forma do cosseno \emph{cancela} além da dobra
$26$, a da meia-corda não); e o limite dimensional: $\mathrm{erro}_n\cdot c_n$ dá $0{,}63$ marcas
em $n=2$ (não fecha), $0{,}08$ em $n=4$ (\textbf{fecha}), e desce daí --- a primeira dimensão que
fecha é a $4$; e o \textbf{teste decisivo} que separa a relação $c_n=\pi_n R_n$ de uma identidade
por construção: o $R$ vem de \emph{outro instrumento} --- a órbita DESENHADA (q passos de aresta
unitária, uma marca de rotação por passo), com o diâmetro medido das coordenadas --- e o $\pi$
geométrico (perímetro contado sobre diâmetro medido) concorda com o da dobra à precisão do próprio
desenho, $|geo-dobra|\le 6\times10^{-12}$, com a órbita a fechar na origem
(\code{tests/luz\_periodo.c} §L6); e o relógio desenha a curva plana decidindo o grau
...
\textbf{Estatuto.} A lei é do \emph{Corpo Universal} (a nilpotência
...
\textbf{Estatuto.} A lei é do \emph{Corpo Universal} (localmente
