...
uma $\pi$; isometria $u(Da,Db)=u(a,b)$ exige $R,R^{\dagger}$
...
\boxed{\,u(Da,Db)=u(a,b)\,}
\]
com $u=2^{-\operatorname{prof}}$. \emph{Só no domínio binário} das leituras
$R,R^{\dagger}$ do \code{fis:thm:B}; travessias arbitrárias e quatro focos
partilhando $\pi$ são outras camadas. \emph{A demonstração fica em}
\code{fisica.tex}; aqui só se classifica o que este documento mede.

\medskip\noindent\textbf{Três situações --- para não confundir.}
\begin{enumerate}\itemsep3pt
\item[\textbf{(I)}] \textbf{Já segue do teorema global} --- citar
...
I & \code{fis:def:arvore}, \code{fis:lem:ultra}; citar teorema global \\
Teor.~\ref{redes:thm-arvore} & II &
isometria de $u$ sob $D$ canónico (Cor.~\ref{redes:cor:troca-u}) \\
...
III & face operacional (\code{fis:def:duomorf}); $D$ canónico não demonstrado \\
Hopfield / Hebb & III & $G_{\mathrm{real}}>1$, combinação aberta --- hipótese extra \\
§RG6 / \code{overwrite} & III & política host; combinação entre incidências aberta \\
...
não demonstra troca dual $R,R^{\dagger}$ nem isometria de $u$; a combinação entre
incidências concorrentes permanece aberta (Prop.~\ref{redes:prop-troca}).
...
\medskip\noindent\textbf{Pergunta aberta (fronteira já registada).}
...
overwrite sim; lei entre incidências concorrentes \textbf{não}. \textbf{P6 permanece TRAVADA.}
...
§RG6 \emph{produziu} --- e não respondeu --- é a fronteira experimental seguinte.
...
\caixa{%
  duas incidências $\longrightarrow$ mesma representação $\longrightarrow$ \textbf{?}%
}

\medskip\noindent\textbf{Família experimental proposta} (sem chamar nenhuma operação de
Hebb antecipadamente):
{\small
\[
\begin{array}{c}
...
\end{array}
\]}
Medir \emph{qual propriedade aparece} em cada degrau --- comutatividade, idempotência,
conservação de medida, efeito em $G_{\mathrm{real}}$ --- antes de nomear a operação.

\medskip\noindent\textbf{P6 --- TRAVADA; pergunta algébrica reservada.}
Quando (e só quando) a família experimental exigir, abre-se a pergunta
\[
C\colon X\times X \to X,
\qquad
C(x_1,x_2)=?
\]
com requisitos mensuráveis: comutatividade $C(a,b)=C(b,a)$? idempotência $C(a,a)=a$?
associatividade? conservação de medida? compatibilidade com $\pi$? efeito em
$G_{\mathrm{real}}$? operação dual/inversa? \emph{Se} dessa análise sair uma soma
Hebbiana, Hopfield entra como \textbf{realização de uma lei já encontrada} --- não
como peça colocada porque «redes neurais precisam de Hopfield».

\caixa{%
  qual lei de combinação é compatível com a arquitetura já construída?%
}

\medskip\noindent\textbf{Escada medida deste documento.}
\caixa{%
  $G=1$ $\to$ $G_{\mathrm{visit}}$ $\to$ $G_{\mathrm{real}}$ $\to$ $G>1$ $\to$
  múltiplos focos $\to$ mesma representação $\to$ overwrite $\to$ \textbf{P6 (TRAVADA)}%
}
...
$\downarrow$\\
overwrite determinado; combinação \textbf{ABERTA}\\
$\downarrow$\\
família experimental (§\ref{redes:fronteira})\\
$\downarrow$\\
pergunta $C(x_1,x_2)=?$\\
$\downarrow$\\
\textbf{P6 / Hopfield --- só se a lei sair da medição}}
\end{minipage}
\end{center}
