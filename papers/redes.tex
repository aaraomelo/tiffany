...
%
%  P6 é postulado de fechamento: autoriza X×X → X. Não se mede; avalia-se contra ele.
...
\gkcapa{Redes Neurais Multifocais}
       {P6 é postulado de fechamento da arquitectura}
       {autoriza $X\times X\to X$; as realizações avaliam-se contra ele}
\maketitle

\begin{abstract}\noindent
\pt{6} não é consequência dos invariantes anteriores. É o decreto que autoriza
a passagem $X\times X\to X$. Uma vez adotado, as realizações avaliam-se contra
ele. A identificação é Hebb; Hopfield dual é a memória da volta.
\[
\boxed{\text{\pt{6} é postulado de fechamento da arquitectura.}}
\]
\[
...
\lambda^{+}+\lambda^{-}=0
\;,\qquad
\text{Hopfield}=\text{memória da volta}.
\]
Negá-lo não refuta uma consequência particular: nega a operação de composição
sobre a qual a arquitectura unifica as realizações matemáticas e físicas.
\end{abstract}

% ═══════════════════════════════════════════════════════════════════════════
\section{Postulado}\label{redes:regra}

\begin{postulado}[\pt{6} --- fechamento]\label{redes:post:p6}\label{redes:post:hebb}\label{redes:post:niveis}\label{redes:post:overwrite}
\pt{6} é postulado de fechamento da arquitectura. Não segue de
$\tau$, $D_{\mathrm{can}}$, $\operatorname{prof}$ nem $u$. É o decreto que
autoriza
\[
X\times X\longrightarrow X.
\]
A operação assim autorizada é $C$, identificada com Hebb
(\mf{postulados}{P6}):
\[
\boxed{C=W,\qquad W_{ij}=\sum_{p}\xi_i^{p}\xi_j^{p}.}
\]
Hopfield dual é a memória da volta (\code{thm:rede-dual}):
\[
\lambda^{+}+\lambda^{-}=0,\qquad F_H\circ F_P=\mathrm{id}.
\]
Uma vez adotado, toda realização posterior avalia-se contra ele.
\end{postulado}
...
Não se afirma que \pt{6} foi medida. Afirma-se: \pt{6} é assumida como
condição de fechamento da construção. Negá-la não refuta uma consequência
particular; nega a operação de composição sobre a qual a arquitectura
pretende unificar as suas realizações matemáticas e físicas.
...
\begin{proof}
Postulado~\ref{redes:post:p6} e \code{fis:thm:interfere}.
\end{proof}
...
(\code{fis:obs:relogio-dual}). Isso é o relógio. A combinação já é \pt{6}.
\end{corolario}
...
Hopfield / Hebb & --- & \pt{6} fecha: $X\times X\to X$; \emph{não} segue do teorema \\
§RG6 / \code{overwrite} & --- & realização contra \pt{6} (Cor.~\ref{redes:cor:ow}) \\
...
\textbf{Hopfield} & soma Hebb (\pt{6}) & $G>1$ & \textbf{sim} --- Post.~\ref{redes:post:p6} \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{Hopfield dual deriva de \pt{6}}\label{redes:hopfield-futuro}
...
\begin{proof}
Postulado~\ref{redes:post:p6} e Cor.~\ref{redes:cor:dual-gh}.
\end{proof}
...
interferência & \code{fis:thm:interfere} & \pt{6} Hebb & Post.~\ref{redes:post:p6} \\
...
\noindent \pt{6} autoriza $X\times X\to X$ e nomeia $C=W$. Não se arranca $C$
dos invariantes. No slot único, Hebb degenera (Cor.~\ref{redes:cor:ow}):
\[
X_0 \xrightarrow{F_1} X_1 \xrightarrow{F_2} X_2,
\qquad
\text{$X_2$ sobrevive},\quad \text{$X_1$ desaparece}.
\]
A família C0--L7 (\code{tests/redes\_combinacao.js}) é realização testável
contra o postulado; não compete com ele. \emph{Não} se promove
...
$C=W$ lê padrões (Post.~\ref{redes:post:p6}).
\end{proposicao}

\begin{proposicao}[incidência não é Hebb]\label{redes:prop-zetamu}
$\zeta$ acumula visitas e $\mu$ deconvolve. Hebb soma padrões numa matriz.
\end{proposicao}

\caixa{%
  \pt{6} fecha a arquitectura: $X\times X\to X$;\\
  $C=W$; Hopfield dual $=\lambda^{+}+\lambda^{-}=0$;\\
  overwrite $=$ Hebb de capacidade $1$.%
}
