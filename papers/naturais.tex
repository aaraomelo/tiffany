...
\quad\neq\quad
\operatorname{Star}(\mathbb{N})=\varnothing
\quad\neq\quad
\text{o que a Física constrói}
\;}
\]
\(\mathcal{D}=\texttt{N/A}\) neste degrau (sem oposto; o transporte reconstrói \(+\), não troca magmas). Cadeia: \(F_8\to\mathbb{N}\to\mathbb{Z}\to\mathbb{Q}\to\mathbb{R}\).
...
\begin{observacao}[o Corpo Natural, e o que não é]
\texttt{Corpo Natural} é o nome estrutural desta obra. \textbf{Estatuto:}
$\mathbb{N}$ não é corpo; $F_8=\mathrm{GF}(2^{8})$ é, e $F_8\neq\mathbb{N}$.
Não se introduzem inversos negativos nem divisão. A passagem ao inteiro fecha
por \textbf{opostos}, não por $\mathcal{D}$.

A equação $x^{2}=x+1$ não tem raiz em $\mathbb{N}$:
$\operatorname{Star}(\mathbb{N})=\varnothing$ no suporte.
\code{fis:thm:estrela} como derivação das duas faces pede oposto: falta, logo
essa aplicação é \texttt{N/A}. Os duais deste paper ($x^{*}$, $T+T^{*}$) são
homónimos de $\mathcal{D}$. \code{fis:thm:quarteto} \texttt{N/A}. Completude:
\texttt{N/A}.
\end{observacao}
