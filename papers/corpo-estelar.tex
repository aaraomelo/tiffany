...
dimensões, o corpo estelar, a geometria, a equação de campo, a cosmologia e a máquina que corre
tudo isto. A derivação corre inteira em por-unidade, sem uma constante física --- e as constantes
vêm no \emph{fim}, quando se veste a roupa, porque uma derivação que precisasse do valor para chegar
ao valor não seria derivação. \textbf{A estrela é a borda e o buraco
negro é o cristal}: não por semelhança, mas por serem os dois sinais de uma mesma quantidade, com o
zero entre eles. Da exigência de conservação derivam-se o coeficiente $\tfrac12$ da equação de
campo, a dimensão quatro, o termo cosmológico como constante de integração e o expoente da força
...
mesmo objecto em operação: \textbf{um corpo estelar não compara --- irradia}, e é por isso que ordena
sem apagar um bit.
\end{abstract}
...
\begin{tabular}{@{}cllll@{}}
\toprule
o sinal & o regime & o que faz & na expansão & na máquina \\
\midrule
$-1$ & \textbf{cristal} & encolhe & contrai & determinista \\
$\phantom{-}0$ & \textbf{borda} & \textbf{conserva} & estacionário & \textbf{reversível} \\
$+1$ & \textbf{caos} & cresce & expande & dissipativo \\
\bottomrule
\end{tabular}
\end{center}

\noindent \emph{Não há quarto regime}, e isso não é uma observação: é o cardinal do conjunto dos
sinais. A borda não é um caso intermédio entre os outros dois --- é o ponto fixo da involução que os
troca, e é por isso que é uma e não um intervalo.
...
\begin{teorema}[e são o par]
A involução $a\mapsto-a$ satisfaz $\nu\circ\nu=\mathrm{id}$, leva cristal em caos e caos em cristal,
e tem \textbf{um} ponto fixo: a borda.
\end{teorema}

\noindent \textbf{Logo estrela e buraco negro não são dois objectos que se parecem}: são os dois
sinais da mesma quantidade, e a borda é o zero entre eles. É o mesmo ponto fixo do trial, e a mesma
involução de toda a estrutura. Um corpo estelar é, exactamente, \emph{um corpo que se mantém na
borda}.
...
$1001$ desvios com \emph{um} ponto fixo; $p=\rho/3$ por contagem de eixos; e o \textbf{controlo}:
...
$=0$ & $f''=0$ & a reta & parabólica --- o potencial \\
...
\subsection*{O termo cosmológico é a constante de integração}

A métrica conserva sozinha, logo $\beta\,g_{\mu\nu}$ entra na equação sem custo e o seu coeficiente
fica \emph{livre} --- foi por isso que sobrou. E o conteúdo que lhe corresponde tem $p=-\rho$, isto
é, $w=-1$: \textbf{o ponto fixo da cosmologia}. Não é um termo acrescentado nem um ajuste: é o que a
integração deixa, e o seu valor de estado é o vácuo. A equação completa é
\[
  \boxed{\;R_{\mu\nu}-\tfrac12R\,g_{\mu\nu}+\Lambda\,g_{\mu\nu}=T_{\mu\nu}\;}
\]
...
e o seu $w$ é $-1$ exacto; o expoente da força foi \textbf{varrido} de $0$ a $6$ em cinco dimensões
...
valor. O invariante que a expansão conserva é a razão do §6 --- o que empurra contra o que puxa ---
...
duas involuções a refletirem em centros diferentes ($0$ e $-1$); $w\mapsto-w$ é o simétrico e é
involução em $17$ graus, verificado pela definição e não pela contagem; e $w+1$ sobe para
$\tfrac23$ \emph{sempre por baixo}, com a distância a valer exatamente $16/(9D)$ em $12$ graus
...
Tudo isto corre, e corre numa máquina que é o mesmo objecto.

\subsection*{Uma instrução}

\[
  \texttt{MOVE(slot, sentido)} : \qquad +1 \ \text{lê}, \qquad -1 \ \text{escreve}.
\]
\textbf{É a Lei 1 aplicada à entrada e à saída}: ler e escrever são a mesma operação com o sinal
trocado. Não há instrução de leitura e instrução de escrita --- há uma, e um sinal.
...
\emph{não há RAM}. O estado vive no disco, endereçado por \code{MOVE}, e o que resider é a régua ---
...
o \textbf{trial} & cristal / borda / caos & menor / igual / maior, por \emph{sinal} \\
...
o corpo estelar & $a=\text{empurra}/\text{puxa}-1$ & estrela na borda, buraco no cristal \\
a geometria & $\Delta=\operatorname{tr}^2-4\det$ & três curvaturas, pelo sinal \\
a equação de campo & conservar & $\alpha=\tfrac12$; $d=4$; $\Lambda$ livre \\
a expansão & $H^2=1/D$ & quatro equações, duas independentes \\
a cosmologia & $\rho=a^{-3(1+w)}$ & três conteúdos; órbita de quatro \\
...
a máquina & \code{MOVE(slot,\,sentido)} & sem RAM, sem tempo, reversível \\
