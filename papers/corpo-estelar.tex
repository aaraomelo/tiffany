...
dos dois relógios do Teorema~\ref{thm:isocrona}: a reactância é exactamente o tempo $n^2$ do lado
\emph{discreto} (Hurwitz) --- \emph{dois lances contra sessenta e quatro} ---, a tábua que se armazena
e se devolve. E esse $64$ é a \textbf{dobra temporal do relógio}: $8^2=(2^3)^2=2^6$, o relógio dobrado
\textbf{seis vezes} --- cada dobra um factor de dois no tempo. O lado \emph{contínuo} (Gentil), pela
norma, não dobra: um escalar, e corre $\mathrm{fp}=1$. A estrela, que não armazena nada
...
fixo entre os dois, indecidível só quando se olha de um lado.
\end{corolario}
...
\textbf{não oscila}: a entrada e a saída dos dados são o próprio movimento (a modulação que gira o
rotor, \S\ref{sub:inversor}), não um sinal guardado para ser lido depois.
...
a plena no cimo. A estrela é o topo de uma e a ponte da outra, e é o mesmo objecto: o ponto fixo,
