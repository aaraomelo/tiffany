...
Não são duas coisas que coincidem: são a \emph{mesma} quantidade lida de dois lados. Postos os
elementos em $+1$ e $-1$, com $|A|+|B|=n$:
\[
  \text{corte} = |A|\,|B|, \qquad
  \text{massa} = \underbrace{n}_{\textstyle\sum x^2} - \frac{(|A|-|B|)^2}{n} ,
\]
e \textbf{as duas são função do mesmo desequilíbrio}. Ambas são máximas onde ele é \emph{zero}, onde
$|A|=|B|$. Logo \textbf{maximizar o corte é maximizar a massa}.

\smallskip
\noindent \textbf{E dizer ``massa na borda'' é dizê-lo duas vezes.} A massa é o que \emph{fecha} a
órbita, e dos três regimes só a borda fecha --- fora dela ou dissipa ou colapsa, e em nenhum dos
casos há volta a fechar. \emph{Só há massa na borda}, logo a qualificação já está dentro da palavra:
\textbf{massa $=$ corte}, e mais nada.

\smallskip
\noindent E a meia volta põe o corte exactamente lá, em todas as escalas: \emph{sem procurar o
máximo e sem comparar candidatos --- rodando}.
...
máximo no \emph{mesmo} argumento em três escalas; e dos três regimes só a borda fecha órbita, logo
``massa na borda'' é redundante; e o \textbf{controlo}:
no triângulo o melhor corte fica em $2$ de $3$ (\code{tests/maxcut.c}).}
...
o max-cut & $|A||B|=\sum(x-c)^2$ & \textbf{massa $=$ corte} --- e só há massa na borda \\
\bottomrule
\end{tabular}
\end{center}
