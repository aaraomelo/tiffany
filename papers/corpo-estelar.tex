...
tudo isto. Não há constante física em lado nenhum --- trabalha-se em por-unidade, onde cada
constante vale $1$ por construção e o que sobra são razões. \textbf{A estrela é a borda e o buraco
...
como codimensão. Fecha-se com a ordenação, que é o mesmo objecto em operação: \textbf{um corpo
estelar não compara --- irradia}, e é por isso que ordena sem apagar um bit.
\end{abstract}
...
Integrando $\rho'+3H(\rho+p)=0$ com $p=w\rho$:
\[
  \boxed{\;\rho\propto a^{-3(1+w)}\;}
\]
e o expoente não é livre --- é o que a continuidade dá, e nenhum outro deixa $\rho\,a^{3(1+w)}$
invariante.
...
\medido{a lei de diluição derivada da continuidade dá $a^{-4}$, $a^{-3}$ e constante para os três
...
a cosmologia & $\rho\propto a^{-3(1+w)}$ & três conteúdos; órbita de quatro \\
