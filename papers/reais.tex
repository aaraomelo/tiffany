...
\noindent\textbf{Camada N--R}, não a Parte Análise, não \(\operatorname{Star}(\mathcal{U})\).
\[
\boxed{\;
\operatorname{Star}(\mathcal{U})=\mathcal{D}
\quad\neq\quad
\operatorname{Star}(\mathbb{R})=\{\varphi,\psi\}
\quad\neq\quad
\text{o que a Física constrói}
\;}
\]
Passagem por \textbf{completação}, não por \(\mathcal{D}\). \(x^{2}=-1\) recusado pela ordem. O corte \textbf{não} é o paper analítico (\(\tau=+1\)).
...
$2$  & $\Q\!\to\!\R$ & o \textbf{corte} (completação)
     & a \textbf{enumerabilidade} & complementar da representação $(A,B)$; involução: $\nu(x)=-x$
     & \code{reais.tex} \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{1} & dual ($1\dg=-1$) & $\nu(x)=-x$ em $(\R,+)$. O par $(A,B)$: complementar da representação, não involução de $\R$ & explícita & \ref{sec:corte} \\
...
\]
--- \textbf{Lei 4} ($T+T^{*}$): duplica-se $\Q$ nas duas classes. $A$ é a classe
inferior ($q^{n}<a$, aberta); $B=\Q\setminus A$ é a classe superior. A operação
$(A,B)\mapsto(B,A)$ é o \textbf{complementar da representação} --- troca a
orientação do par e \textbf{não} permanece no espaço orientado dos cortes, nem
é involução de $\R$. A involução de $(\R,+)$ é $\nu(x)=-x$, noutro sítio.
\textbf{Lei 2}: $B=\Q\setminus A$ duas vezes devolve o mesmo par não ordenado
$\{A,B\}$. A decisão é \textbf{inteira}: $q=p/d$ está em $A$ conforme
...
\medskip\noindent\textbf{E o que a desigualdade \emph{significa}}, que a
varredura não mostra: dela sai que as bolas de raio fixo \textbf{particionam} ---
duas ou coincidem ou são disjuntas --- e que todo o ponto de uma bola é seu
centro. É esta a diferença estrutural face à euclidiana, onde as bolas do mesmo
raio se cruzam parcialmente; a desigualdade é a forma escrita, a partição é o
conteúdo. Demonstrado em \code{papers/aranha.tex}, que é autocontido, junto com
a consequência computacional: numa realização sobre a árvore, a célula
\emph{é} a bola, e por isso a contagem por célula não precisa de cuidado nenhum
com sobreposições --- não há sobreposição a haver.
...
\begin{observacao}[o Corpo Real, e o que não é]
\texttt{Corpo Real} é o nome estrutural desta obra. \textbf{Estatuto:}
corpo ordenado completo --- unicidade (Teorema~\ref{thm:unico}) \emph{depois}
do corte (Def.~\ref{def:corte}), não como definição.
A passagem $\mathbb{Q}\to\mathbb{R}$ fecha por \textbf{completação}, não por
$\mathcal{D}$ e não por isomorfismo ($\mathbb{Q}\not\cong\mathbb{R}$).

A involução de $(\mathbb{R},+)$ é $\nu(x)=-x$, $\nu\neq\mathcal{D}$,
$\nu\neq\star$. O par $(A,B)$ com $B=\mathbb{Q}\setminus A$ admite o
complementar da representação $(A,B)\mapsto(B,A)$: troca orientação, não é
involução do espaço orientado dos cortes nem do corpo.
$x^{2}=-1$ é recusado pela ordem. $\operatorname{Star}(\mathbb{R})=\{\varphi,\psi\}$,
lei complementar da cadeia, não analogia com Alonzo.
\end{observacao}
...
\item[\textbf{1}] o complementar da representação $(A,B)\mapsto(B,A)$ troca a orientação; a involução de $(\R,+)$ é $\nu(x)=-x$.
