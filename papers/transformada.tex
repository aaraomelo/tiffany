...
\begin{abstract}
O objeto aqui construído não é uma nova aritmética: é uma ponte entre duas leituras do mesmo suporte discreto. A família de polígonos regulares aproxima a circunferência em sentido geométrico apropriado; a cadeia
\[
...
\]
O ponto decisivo é separar suporte, leitura e métrica. O círculo e a reta não são duas fases de um mesmo objeto global; são duas representações compatíveis do mesmo suporte em escalas diferentes. A métrica lê a concordância estrutural do suporte; a distribuição só surge depois da multiplicidade e da normalização. Assim, a carta local não produz probabilidade nem converte literalmente o círculo em reta. Ela apenas lineariza a geometria local.
...
\begin{definicao}[duas leituras de um mesmo sistema]
...
Estas duas leituras são compatíveis, mas não idênticas. O círculo carrega a leitura angular global; a reta é a leitura métrica local.
\end{definicao}
...
\]
Este cálculo mostra que a corda é localmente assintótica à métrica linear. A construção da reta real exige, porém, uma etapa adicional: o completamento da métrica linear. Em termos precisos,
\[
\overline{R\mathbb Q}\cong\mathbb R.
\]
A reta não é a imagem literal do círculo. Ela é o fechamento ordenado e métrico da leitura linear local que nasce na carta $x=R\theta$.
...
A geometria local e o dual espectral são, portanto, duas realizações diferentes da mesma ideia estrutural:
\[
\boxed{
\begin{array}{ccc}
\theta
...
\]
sem cometer o salto ilegítimo $P_n\to\mathbb R$. Em outras palavras, ele mostra que a leitura linear não é a imagem literal do círculo, mas a realização local compatível com a métrica cordal; e que a leitura espectral não é uma nova entidade, mas outra representação do mesmo suporte. Neste quadro, a probabilidade não é tomada como princípio primeiro; ela é a normalização do peso contado sobre o suporte. O papel da carta local é compatibilizar escalas e leituras, não produzir distribuição por si só.

O mesmo princípio aparece no dual do repositório com uma forma mais precisa: a transformada universal é a avaliação nas folhas do corpo,
\[
