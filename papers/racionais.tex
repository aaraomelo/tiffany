...
\quad\neq\quad
\operatorname{Star}(\mathbb{Q})=\varnothing
\quad\neq\quad
\text{o que a Física constrói}
\;}
\]
Passagem por \textbf{quocientes}, não por \(\mathcal{D}\). O inverso \(\iota\neq\mathcal{D}\). Completude: o degrau seguinte.
...
pelos cortes de $\mathbb{Q}$. \textbf{$\mathbb{Q}$ não é completo}: a
completude não é propriedade deste degrau. Na linguagem da torre
...
\begin{observacao}[o Corpo Racional, e o que não é]
\texttt{Corpo Racional} é o nome estrutural desta obra. \textbf{Estatuto:}
corpo (Teorema~\ref{thm:q}). A passagem $\mathbb{Z}\to\mathbb{Q}$ fecha por
\textbf{quocientes} (localização), não por $\mathcal{D}$.
$n\mapsto[(n,1)]$ é inclusão. $\iota$ induz o inverso, $\iota\neq\mathcal{D}$.
$\operatorname{Star}(\mathbb{Q})=\varnothing$.
Completude: \textbf{não}; a completação é o degrau \code{reais.tex}.
\end{observacao}
