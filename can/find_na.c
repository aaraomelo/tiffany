...
#define SLOT_CONST_0        48
#define SLOT_CONST_1        49
#define SLOT_CONST_FF       50
#define SLOT_SIGN_BIT       51
...
int main(void){
  ...
  fmem[SLOT_CONST_0]=0;
  fmem[SLOT_CONST_1]=1;
  fmem[SLOT_CONST_FF]=0xFF;
  fmem[SLOT_SIGN_BIT]=0x80;
  ...
  fmem[SLOT_NA_VALUE_LO+1]=0x12;

  uint8_t payload[8] = {0x34, 0x12, 0,0,0,0,0,0};
  for(int i=0;i<8;i++) fmem[SLOT_PAYLOAD_LO+i] = payload[i];

  init(fmem);
  printf("=== Full trace (all steps, verbose) ===\n");
  for(int i=0;i<500&&!halted;i++){
    uint8_t op=bin[PC];
    printf("PC=%3d op=0x%02x A=0x%04x B=0x%04x R=0x%04x flags=%d | mem[48..51]=[0x%04x,0x%04x,0x%04x,0x%04x] mem[8..15]=[",
      PC,op,A,B,R,flags, mem[48],mem[49],mem[50],mem[51]);
    for(int j=8;j<16;j++) printf("0x%04x%s", mem[j], j<15?",":"");
    printf("]\n");
    step(0);
  }
  ...
  printf("\nERG: rc=0x%02x status=0x%02x raw=0x%016llx\n", rc, status, (unsigned long long)raw);
  printf("Expected: rc=0x00 status=0x01 raw=0x1234\n");
