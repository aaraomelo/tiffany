/* diag_erg_load.c — trace LOAD/STORE with actual binary dump */
#include <stdio.h>
#include <stdint.h>
#include <string.h>
#include <stdlib.h>
...
int main(void) {
    FILE *f = fopen("can/diag_erg2.bin", "rb");
    if (!f) { printf("Cannot open can/diag_erg2.bin\n"); return 1; }
    fseek(f, 0, SEEK_END);
    int binlen = ftell(f);
    fseek(f, 0, SEEK_SET);
    ...
    while (pc < binlen) {
        uint8_t op = bin[pc];
        printf("Offset %d (0x%02x): 0x%02x (%s)\n", pc, pc, op, opname(op));
        switch (op) {
            case 0x00: pc += 1; break;
            case 0x01: printf("  operand: 0x%02x (%d)\n", bin[pc+1], bin[pc+1]); pc += 2; break;
            case 0x02: printf("  operand: 0x%02x (%d)\n", bin[pc+1], bin[pc+1]); pc += 2; break;
            case 0x03: pc += 1; break;
            case 0x04: pc += 1; break;
            case 0x05: pc += 1; break;
            case 0x06: printf("  rel: 0x%02x%02x (%d)\n", bin[pc+1], bin[pc+2], (int16_t)(bin[pc+1] | (bin[pc+2] << 8))); pc += 3; break;
            case 0x07: printf("  rel: 0x%02x%02x (%d)\n", bin[pc+1], bin[pc+2], (int16_t)(bin[pc+1] | (bin[pc+2] << 8))); pc += 3; break;
            case 0x08: printf("  rel: 0x%02x%02x (%d)\n", bin[pc+1], bin[pc+2], (int16_t)(bin[pc+1] | (bin[pc+2] << 8))); pc += 3; break;
            case 0x09: pc += 1; break;
            case 0x0A: pc += 1; break;
            case 0x0B: printf("  operand: 0x%02x (%d)\n", bin[pc+1], bin[pc+1]); pc += 2; break;
            default: pc += 1; break;
        }
    }
    printf("\n");
    ...
    /* Set up test data */
    fmem[40] = 0x0034;  /* slot 40 = payload[0] */

    run(100);

    printf("\n=== FINAL STATE ===\n");
    printf("A=0x%04x B=0x%04x R=0x%04x PC=%u halted=%d\n", A, B, R, PC, halted);
    printf("fmem[40] = 0x%04x (input)\n", fmem[40]);
    printf("fmem[50] = 0x%04x (STORE target, expected 0x0034)\n", fmem[50]);
