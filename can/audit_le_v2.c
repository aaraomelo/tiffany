...
int main(void) {
    ...
    int fd = open("/c/tmp/erg_le.dat", O_RDWR | O_CREAT | O_TRUNC, 0644);
    if (fd < 0) { printf("Cannot open /c/tmp/erg_le.dat\n"); return 1; }
    ...
    /* Slot 40..47 = payload bytes (each slot = 1 byte) */
    Word8 payload[8] = {0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0};
    for (int i = 0; i < 8; i++) {
        slot_mem_grava(fd, 40 + i, payload[i]);
    }

    /* Verify payload */
    printf("=== PAYLOAD VERIFICATION ===\n");
    for (int i = 0; i < 8; i++) {
        Word8 v = slot_mem_le(fd, 40 + i);
        printf("  slot %d = 0x%02x (%s)\n", 40+i, v, v == payload[i] ? "OK" : "FAIL");
    }
    printf("\n");
    close(fd);
    ...
    /* Read results */
    fd = open("/c/tmp/erg_le.dat", O_RDONLY);
    if (fd < 0) { printf("Cannot open result file\n"); return 1; }

    printf("=== OUTPUT ===\n");
    printf("raw_out slots 24..31:\n");
    for (int i = 0; i < 8; i++) {
        Word8 v = slot_mem_le(fd, 24 + i);
        printf("  slot %d = 0x%02x\n", 24+i, v);
    }
    Word8 ret_code = slot_mem_le(fd, 32);
    Word8 status = slot_mem_le(fd, 33);
    printf("return_code (slot 32) = 0x%02x\n", ret_code);
    printf("status (slot 33) = 0x%02x\n", status);
    close(fd);

    /* Verify LE extraction */
    printf("\n=== VERIFICATION ===\n");
    /* LE: payload[0]=0x12 at slot 24, payload[1]=0x34 at slot 25, etc. */
    ...
    for (int i = 0; i < 8; i++) {
        Word8 v = slot_mem_le(fd, 24 + i);
        if (v != expected[i]) {
            printf("FAIL slot 24+%d: expected 0x%02x, got 0x%02x\n", i, expected[i], v);
            pass = 0;
        }
    }
