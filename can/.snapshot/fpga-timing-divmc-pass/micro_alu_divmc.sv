...
module micro_fractal #(
    ...
    typedef enum logic [3:0] {
        ...
        S_ALU,    /* ALU    (exceto DIV) */
        S_DIV_IT, /* DIV iterativo (1 bit por ciclo) */
        S_DIV_END,/* DIV concluida: canal <- div_q */
    ...
    /* datapath da divisao longa iterativa (microarquitetura do IOP_DIV) */
    logic [63:0]  div_a, div_b;            /* numerator/denominator latched */
    logic [64:0]  div_r;                   /* remainder  (inv: div_r < 2*div_b) */
    logic [63:0]  div_q;                   /* quotient */
    logic [5:0]   div_cnt;                 /* bit 63..0 */
     ...
     * IOP_DIV escreve o quociente div_q (STORE r = DIVQ(a,b) = canal final).
     * ------------------------------------------------------------------ */
    wire [IDX_W-1:0] mwaddr = d_wr ? d_addr : rr[IDX_W-1:0];
    wire [63:0]      mwdata = d_wr ? d_din  : ((rop == IOP_DIV) ? div_q : alu_r);
     ...
     * O caso IOP_DIV nunca e usado para resultado (mwdata usa div_q); o
     * divisor combinacional `a/b` nao existe mais no netlist (dead, otim).
     * ------------------------------------------------------------------ */
    always_comb begin
        case (rop)
            ...
            IOP_DIV: alu_r = 64'd0;               /* inerte: DIV vai p/ o loop */
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            case (state)
                ...
                S_ALU: begin
                    if (rop == IOP_DIV) begin
                        /* DIV(a,0) = 0 imediato (contrato DIVQ) */
                        if (b_tmp == 64'd0) begin
                            div_q <= 64'd0;
                            canal <= 64'd0;
                            state <= S_WB;
                        end else begin
                            /* long division: r = {div_r<<1, a[63]}; ... */
                            div_a   <= a_tmp;
                            div_b   <= b_tmp;
                            div_r   <= 65'd0;
                            div_q   <= 64'd0;
                            div_cnt <= 6'd63;   /* comeca pelo bit 63 */
                            state   <= S_DIV_IT;
                        end
                    end else begin
                        canal <= alu_r;
                ...
                S_DIV_IT: begin
                    /* um bit de DIV a cada ciclo (equivalente ao for i=63..0
                     * de micro.c/DIV: r = (r<<1)|bit; if (r>=b) {r-=b; q[i]=1;} )
                     * r é 65 bits (inv: r < 2*b, cabe em 65 bits). */
                    if (({div_r[63:0], div_a[div_cnt]} >= {1'b0, div_b})) begin
                        div_r <= {div_r[63:0], div_a[div_cnt]} - {1'b0, div_b};
                        div_q[div_cnt] <= 1'b1;
                    end else begin
                        div_r <= {div_r[63:0], div_a[div_cnt]};
                    end
                    if (div_cnt == 6'd0)
                        state <= S_DIV_END;
                    else
                ...
                S_DIV_END: begin
                    canal <= div_q;       /* q completo: canal <- DIVQ(a,b) */
                    state <= S_WB;
                end

                S_WB: begin
                    /* STORE r = MOVE(r, -1); canal <- r (STORE_VAL) */
                    /* word[rr] <= (alu_r | div_q) escrito pela porta unica */
                    /* canal permanece = valor STOREd (STORE_VAL) */
                    state <= S_PCU;
                end
