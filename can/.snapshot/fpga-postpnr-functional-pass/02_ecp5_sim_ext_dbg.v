	...
	function [17:0] read_core;
		...
		begin
			...
			case (dw)
				...
				2: read_core[1:0] = mem[a[13:5]][a[3:0]*2 +: 2];
				4: read_core[3:0] = mem[a[13:5]][a[2:0]*4 +: 4];
...
if (rsta_eff) begin
			...
			if (wea_eff) begin
`ifdef EBR_DEBUG
				$display("%m eeA t=%0t wea=%b ada=%04x dia=%03x read=row%0d/lane%0d", $time, wea_eff, ada, dia, ada[13:5], ada[4:3]);
`endif
				case (DATA_WIDTH_A)
					1: mem[ada[13:5]][ada[4:0]] <= dia[0];
					2: mem[ada[13:5]][ada[3:0]*2 +: 2] <= dia[1:0];
					4: mem[ada[13:5]][ada[2:0]*4 +: 4] <= dia[3:0];
		...
		if (rstb_eff) begin
			...
			if (web_eff) begin
				case (DATA_WIDTH_B)
					...
					2: mem[adb[13:5]][adb[3:0]*2 +: 2] <= dib[1:0];
					4: mem[adb[13:5]][adb[2:0]*4 +: 4] <= dib[3:0];
...
`ifdef EBR_DEBUG
			$display("%m eeB t=%0t adb=%04x rdval=%03x (prev_dob=%03x) row=%0d lane=%0d", $time, adb, read_core(DATA_WIDTH_B, adb), dob_lat, adb[13:5], adb[4:3]);
`endif
		end
	end
