...
int main(void) {
    FILE *f = fopen("can/diag_load40.bin", "rb");
    if (!f) { printf("Cannot open can/diag_load40.bin\n"); return 1; }
    fseek(f, 0, SEEK_END);
    int binlen = ftell(f);
    fseek(f, 0, SEEK_SET);
    ...
    printf("mem[40] = 0x%04x (input)\n", mem[40]);
    printf("mem[50] = 0x%04x (STORE target, expected 0x0034)\n", mem[50]);
