 ...
 *
 * O Aarão: "veja, SABEMOS A CIFRA. Manda ele ir dizendo o que sabe e vamos cifrando no lugar
 * certo, via embeddings e corpo diferencial." E depois: "ou seja, pegar os embeddings dele e
 * cifrar — certamente tem um lugar. A CIFRA JÁ AUTOCOMPLETA."
 *
 * E ISTO FECHA A SÉRIE INTEIRA. O `folhas.c` procurava a régua que punha o ponto na unidade,
 * varrendo (B,C) — e a varredura escolhia sempre a mesma folha. Mas não era preciso varrer nada:
 * **a cifra do ponto É o endereço dele**. A fração contínua não se escolhe, calcula-se; e ela
 * autocompleta, que é o que Lagrange diz — periódica se e só se quadrática.
 *
 *      ele FALA        →  o embedding          (768 coordenadas, dele)
 *      nós CIFRAMOS    →  a fração contínua    (não se escolhe: calcula-se)
 *      a cifra É       →  o endereço           e o lugar já estava lá
 *
 * *Não é selecionar, não é classificar, não é decidir. É calcular o endereço que o ponto já tem.*
 *
 *   §Q1  ele fala, nós ciframos — e a cifra é DELE, não de uma escolha nossa
 *   §Q2  a cifra é o ENDEREÇO: mesma frase, mesmo lugar; frases diferentes, lugares diferentes
 *   §Q3  a cifra AUTOCOMPLETA — os primeiros termos determinam o resto, e mede-se quanto
 *   §Q4  e o corpo diferencial: a derivada da cifra, que é o que a faz autocompletar
 *
 *   ./interroga.sh    (com o ollama acordado)
 *   cc -O2 -std=c99 -Wall -Wformat cifrando.c -lm -o cifrando && ./cifrando
 */
...
static void secao_Q1(void){
    ...
    for(int i = 0; i < NV; i++){
        ...
        NC[i] = cifra(fabs(b) > 1e-12 ? a/b : a, CIF[i], MAXC);
        if(i < 8){
            printf("        %-3d  %20.10f    ", i, fabs(b)>1e-12 ? a/b : a);
...
static void secao_Q2(void){
    ...
    /* e o mesmo ponto dá sempre a mesma cifra: recalcula-se e compara-se */
    int instavel = 0;
    for(int i = 0; i < NV; i++){
        double a = 0, b = 0;
        for(int d = 0; d < ND; d += 2) a += V[i][d];
        for(int d = 1; d < ND; d += 2) b += V[i][d];
        int c2[MAXC], n2 = cifra(fabs(b)>1e-12 ? a/b : a, c2, MAXC);
...
static void secao_Q3(void){
    ...
    /* Lagrange: cifra periódica ⟺ quadrático. Mede-se a periodicidade: dado um prefixo, o
     * termo seguinte é previsível se a cifra tiver período. Compara-se com o acaso. */
    ...
    printf("        acertos DENTRO da cifra:   %d de %d   (%.1f%%)\n",
           acertos, tentativas, 100.0*acertos/tentativas);
    printf("        acertos contra OUTRA cifra: %d de %d   (%.1f%%)\n",
           acertos_ctl, tentativas, 100.0*acertos_ctl/tentativas);
    ok("há tentativas para medir", tentativas > 50);
    /* E A MEDIDA DIZ QUE NÃO — 20,2% dentro contra 23,7% contra outra cifra. A cifra destes
     * pontos NÃO autocompleta, e a razão tem nome: **Lagrange**. A fração contínua é periódica
     * SE E SÓ SE o número for quadrático. Um embedding dá um real genérico, não quadrático —
     * logo a cifra dele não é periódica, logo não autocompleta.
     *
     * *A cifra autocompleta no CORPO, onde os números são quadráticos por construção. Não
     * autocompleta num real qualquer que veio de uma rede neural.* As duas metades do pedido
     * separam-se aqui: cifrar no lugar certo FUNCIONA (§Q2, zero colisões, determinista);
     * autocompletar exige que o ponto seja do corpo, e estes não são. */
...
static void secao_Q4(void){
    ...
    /* a derivada: as diferenças entre termos consecutivos. Se a cifra for periódica, a
     * derivada anula-se com o período — e é isso que a faz autocompletar. */
    int zeros = 0, total = 0;
    for(int i = 0; i < NV; i++)
        for(int k = 1; k < NC[i]; k++){ total++; if(CIF[i][k] == CIF[i][k-1]) zeros++; }
    printf("        derivadas nulas (termos repetidos consecutivos): %d de %d  (%.1f%%)\n",
...
int main(void){
    ...
    while(NV < MAXV && getline(&l, &cap, f) > 0){
        ...
        while(d < 768){ double x = strtod(p, &fim); if(fim == p) break; V[NV][d++] = x; p = fim; }
        if(d < 64) continue;
