...
#define LMAXD 64
static int posto(int64_t M[LMAXD][LMAXD], int lin, int col){
    int64_t A[LMAXD][LMAXD];
    ...
    for(int c = 0; c < col && r < lin; c++){
        ...
        if(piv < 0) continue;
        for(int j = 0; j < col; j++){ int64_t t=A[r][j]; A[r][j]=A[piv][j]; A[piv][j]=t; }
        for(int i = r+1; i < lin; i++){
            if(!A[i][c]) continue;
            int64_t a = A[r][c], b = A[i][c];
...
static int posto_bipartido(const int64_t *psi, int N, int j){
    int64_t M[LMAXD][LMAXD];
...
printf("     separável naquele corte se e só se o posto é 1. Amplitudes inteiras.\n\n");
{
    ...
    int64_t ghz[16]; memset(ghz, 0, sizeof ghz);
    ghz[0] = 1; ghz[dim-1] = 1;
...
printf("\n§S3  A soma fecha em zero — e as fases do código já são raízes da unidade.\n\n");
{
    ...
    int64_t partes[7] = {314159, 271828, 161803, 141421, 577215, 693147, 0};
    int64_t soma = 0;
    ...
    printf("      6 partes de clientes + 1 do sistema, em Z_%" PRId64 "\n", p);
    printf("      soma dos sete ................................. %" PRId64 "\n", total);
     ...
     * e isso conta-se em inteiros. A versão anterior somava 16 cossenos em int64_t e
...
{
    const int64_t p = 1000003;
    int64_t s[6] = {314159, 271828, 161803, 141421, 577215, 693147};
    int64_t K = 0; for(int i = 0; i < 6; i++) K = (K + s[i]) % p;   /* a do sistema */
    ...
    int64_t t[6]; const int64_t c = 424242;
    for(int i = 0; i < 6; i++) t[i] = (s[i] + c) % p;
    int igual = 1;
    for(int i = 0; i < 6; i++) for(int j = 0; j < 6; j++){
        int64_t d1 = ((s[i]-s[j]) % p + p) % p, d2 = ((t[i]-t[j]) % p + p) % p;
        if(d1 != d2) igual = 0;
    }
...
{
    const int64_t p = 97;                        /* pequeno, para varrer o espaço inteiro */
    int64_t s[4] = {11, 42, 7, 63};
    int64_t K = 0; for(int i = 0; i < 4; i++) K = (K + s[i]) % p;
    /* sem a parte 3, quantos valores dela são consistentes com as diferenças das outras? */
    int64_t compat_sem_K = 0, compat_com_K = 0;
    for(int64_t v = 0; v < p; v++){
        /* as diferenças entre as partes conhecidas não dizem nada sobre v: toda v serve */
        compat_sem_K++;
        int64_t soma = (s[0] + s[1] + s[2] + v) % p;
        if(soma == K) compat_com_K++;
    }
    printf("      valores possíveis da parte que falta, em Z_%" PRId64 ":\n", p);
    printf("        só com as diferenças das outras ............. %" PRId64 "  (o espaço inteiro)\n", compat_sem_K);
    printf("        acrescentando a chave do sistema ............ %" PRId64 "  (exatamente uma)\n", compat_com_K);
