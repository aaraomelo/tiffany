...
;(async () => {
  ...
  const lazyBytes = {}
  {
    ...
    for (const fonte of ['papers/dualsort.tex', 'papers/corpo-estelar.tex', 'teoria.tex', 'enredo.tex', 'catalogo.tex']) {
    ...
    ok('§D2 o subset lazy é estritamente menor que o manifesto — o wasm não leva o catálogo no dualsort',
      falhou === 0 && lazyBytes['papers/dualsort.tex'] < 2.2e6 && lazyBytes['catalogo.tex'] < 4e6)
  }
  ...
  const tempos = {}
  {
    ...
    const sub = disco.ficheirosPara('papers/dualsort.tex', lista)
    let nLazy = 0
  ...
  let ram = null
  {
    if (!fs.existsSync(WASM)) {
      ...
      /* ─── §D5 fopen miss → 1 ficheiro (Map JS, sem poe-all no DISCO) ─────────── */
      {
        ...
        const enc = Buffer.from('papers/dualsort.tex', 'utf8')
        ...
        ok('§D5 fopen miss → inflate/Map → poe 1: dualsort sem poe prévio, rc=0 e miss>0',
          rc === 0 && tam > 1e5 && missN > 5 && missB < lazyBytes['papers/dualsort.tex'] + 1e5)
