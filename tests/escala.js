...
console.log('\n§T4  O CONTROLO: mudada a escala no estilo.tex, o PDF muda — e volta.\n')
{
  ...
  try {
    compoe('papers/dualsort.tex', '/tmp/t_a.pdf'); antes = tamanhos('/tmp/t_a.pdf')
    ...
    if (mordeu) {
      ...
      compoe('papers/dualsort.tex', '/tmp/t_b.pdf'); depois = tamanhos('/tmp/t_b.pdf')
    }
  } finally {
    fs.writeFileSync(ESTILO, original)                 /* devolve-se SEMPRE */
  }
  compoe('papers/dualsort.tex', '/tmp/t_c.pdf'); voltou = tamanhos('/tmp/t_c.pdf')
