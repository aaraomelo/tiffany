/* conecthus/backends/sql/canonizar.c — SQL realiza «canonizar» (porta da interface).
...
int vaga_compilar(int in_off, int n, int out_off){
    int m = 0, i = 0;
    while(i < n){
...
int vaga_descompilar(int in_off, int n, int out_off){
    int m = 0, i = 0;
    while(i < n){
        if(arena[in_off + i] == '<'){
            int j = i + 1;
...
int vaga_move(int in_off, int n, int out_off, int sentido){
    if(sentido < 0) return vaga_compilar(in_off, n, out_off);
    return vaga_descompilar(in_off, n, out_off);
}
