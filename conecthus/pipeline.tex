...
% Promovida a eixo da arquitetura: papers/arquitetura.tex §sec:pipeline.
% Auto-contido: pdflatex pipeline.tex
% Fonte: eval.txt + fundamento.tex + papers/corpo_peano.tex + arquitetura.tex.
...
tests/fronteira_front.js      medidor UI fronteira
  ...
  fundamento.tex
  pipeline.tex
papers/arquitetura.tex  §pipeline = terceiro eixo
...
papers/conversa_pipeline.tex  corpus
\end{verbatim}
...
\bibitem{arquitetura} A.~M.~Lopes. \emph{A arquitetura}
  (\code{papers/arquitetura.tex}) --- três eixos: porta, torre, pipeline
  (\S\,pipeline); «todo passo reverte, ou paga».
\bibitem{eval} Gerente (GPT). Notas de revisão em \code{eval.txt}
...
\bibitem{peano} A.~M.~Lopes. \emph{Corpo de Peano}
  (\code{papers/corpo\_peano.tex}) --- $\pi_k$, Maestro/Metrónomo,
...
Especificação v0.2 --- eval.txt enquadrado no Corpo de Peano.\\
\href{https://goldenkingdom.patriatechnology.com}{goldenkingdom.patriatechnology.com}.
