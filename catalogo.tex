...
\emph{E a construção subiu de dimensão}, que é o que a teoria pedia: $\Mat{a}$ generaliza-se para a
\textbf{companheira} da borda $x^n = m x^{n-1} + 1$, e o caso $n=2$ é o gato do metal. Três
propriedades sobrevivem em toda dimensão --- $|\det| = 1$; a raiz dominante é real, simples e $>1$
(\emph{Pisot}); e $|\sigma|\prod_{|\lambda|<1}|\lambda| = 1$, que é a conservação entre os níveis.
Daí o teorema: $\R^n$ \textbf{mergulha} em $\R$ por $x\mapsto\sigma$ e \textbf{herda a ordem}, logo
a construção por códigos vale em toda dimensão. \emph{E é aqui que $\R^n$ se separa de $\C$}: a
diferença não é de grau, é a existência de uma raiz real distinguida para onde projetar. A dimensão
abaixo é a sombra da de cima, e a sombra não perde nada. Dez páginas, zero \emph{overfull}, compila
em Ubuntu limpo.
