...
\section{Toda dimensão: a companheira e os corpos \texorpdfstring{$K_{n,m}$}{Knm}}
...
\begin{obs}[a parte que \textbf{não} vale: contraexemplo]\label{obs:naopisot}
Não é verdade que as raízes restantes tenham todas módulo $<1$. Para $m=1$ a propriedade de Pisot
vale apenas para $n\in\{2,3,4\}$ e falha a partir de $n=5$. O caso $n=5$ é explícito:
\[
  x^5-x^4-1 \;=\; (x^2-x+1)(x^3-x-1),
\]
e $x^2-x+1$ tem as suas duas raízes \emph{sobre} a circunferência unitária (são raízes primitivas
sextas da unidade), de módulo exatamente $1$. Para $n=6,\dots,12$ e $m=1$ o segundo maior módulo
...
$\beta_{n,1}$ é redutível sobre $\Q$ exatamente para $n\equiv 5 \pmod 6$ no intervalo medido
($n = 5,11,17,23,29,35$), sempre com o fator ciclotómico $x^2-x+1$. Logo
...
topológica}, e faz o Teorema~\ref{thm:constrn} parecer dizer que a construção da Seção~6 sobe para
...
\emph{E $K_{n,m}$ é só um lado.} O outro é o reticulado dual $L^{*}$, e o que os liga é
\[
  \operatorname{vol}(L)\cdot\operatorname{vol}(L^{*}) \;=\; 1 ,
\]
medido exatamente --- a mesma conservação de $|\det| = 1$ que atravessa este texto desde a
Seção~2 (Corolário~\ref{cor:det}). \textbf{O $\R^n$ ``de facto'' é $L \oplus L^{*}$, e não uma metade.} A construção existe;
o que não existe é ela caber no corpo sozinho.
\end{obs}

\begin{obs}[o enunciado correto de ``toda dimensão'']
O que de facto sobe de dimensão é a Seção~6 na sua forma topológica. O espaço de Baire satisfaz
\[
...
\paragraph{Onde $K_{n,m}$ e $\C$ se separam.} A Seção anterior mostrou que, quando
$\beta_{n,m}$ é irredutível, $K_{n,m}$ mergulha em $\R$ por $x\mapsto\sigma$
(Proposição~\ref{prop:dominante}) e a ordem herda-se. \textbf{É exatamente isso que falta em $\C$}:
não há raiz real distinguida para onde projetar, logo não há ordem \emph{de corpo} para herdar. A
diferença não é de grau nem de dificuldade --- é a existência do mergulho em $\R$.
...
Seção~6.1 não foi escolhida --- caiu de $\det M_n = (-1)^{n+1}$, isto é, de um \emph{sinal}. Sobre
...
como em (i) da Seção~8 --- e isso é barato: qualquer bijeção com $\R$ serviria. O que uma
...
demonstravelmente falsa (Seção~8, item (ii)). O Teorema~\ref{thm:ordemsigma} é sobre $\Sig$ e
...
\noindent A conclusão do projeto estava certa; a prova, não. E o detalhe que fecha a Seção~8: a
...
autodualidade, e a Seção~8 mostra onde as duas se tocam de facto, em
