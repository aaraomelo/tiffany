...
Dois \emph{irracionais} têm caudas coincidentes na expansão se e só se estão na mesma órbita de
$\GL_2(\Z)$. As classes de equivalência da codificação, \emph{restrita aos irracionais}, são as
órbitas da acção.
\end{proposicao}

\begin{obs}
A restrição aos irracionais não é técnica: em $\Pj(\Q)$ todos os racionais formam uma \emph{única}
órbita de $\GL_2(\Z)$, e as suas expansões são finitas --- não há ``caudas'' a comparar. O enunciado
sem essa hipótese seria falso.
\end{obs}
...
\subsection{$J$ troca o zero pelo infinito --- e é o que reinicia o nível}\label{sec:troca}
...
$J = \begin{psmallmatrix}0&1\\1&0\end{psmallmatrix}$ troca $0$ com $\infty$: $J\cdot 0 = \infty$ e
$J\cdot\infty = 0$. Já $\Mat{a} = R^aJ$ leva $0\mapsto\infty$ e $\infty\mapsto a$ --- \emph{não} é
uma troca, é a troca seguida da translação por $a$.
\end{proposicao}
...
$N_B = \begin{psmallmatrix}1&B\\0&-1\end{psmallmatrix}$, para um parâmetro $B$ qualquer, com $N^2 = I$ e $\det N = -1$: \emph{uma
...
(trinta e oito asserções) e \code{tools/cone\_espiral.c} (sete), na bateria do repositório.
