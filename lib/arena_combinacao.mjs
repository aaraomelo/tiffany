 ...
 *
 * Régua teórica (fisica.tex): τ, D_can, prof, u demonstrados; C é P6
 * (fechamento em papers/redes.tex). O host avalia-se contra P6; não o mede.
 *   fis:def:incid        — W = Zc^I, matriz S subdiagonal; zeta/mu
 *   fis:thm:zetamu       — escrita = convolução; recuperação = μ
 *   fis:thm:troca-realizacao — D_can no domínio da árvore; ≠ C; ≠ τ
 ...
 * K0–K7: caracterização dos candidatos (lexMax, lexMin, rectCell) — ainda não f=C.
 * L0–L7: invariantes do suporte — o que restringe a classe admissível de f.
 * M1–M2: autoridade da ordem discreta sobre A_min (isolada de ∂; não promove lexMax).
 * E_∂: covariância sob a dobra (isolada da ordem; não promove lexMax).
 * lexMin: mesma semilattice que lexMax; prova negativa de unicidade no host.
 * C = W por P6 (papers/redes.tex). Não promove lexMax.
 */
...
// ── K0–K7: caracterização (lexMax, lexMin, rectCell) — host não nomeia C ──
 ...
 *  lexMin entra como a outra orientação da mesma semilattice — prova negativa,
 *  não escolha de C. Relógio discreto (marcas finitas), não ℝ. */
export const CANDIDATO = {
  lexMax: { nome: 'lexMax', reducer: PROBE_REDUCER.lexMax, R: '(x1,x2)↦max(x1,x2)' },
  lexMin: { nome: 'lexMin', reducer: PROBE_REDUCER.lexMin, R: '(x1,x2)↦min(x1,x2)' },
...
export function runK6Invariancia (reducer, meta, payloads = K_PAYLOADS) {
  ...
  return {
    ...
    referencia: 'K6: mesmo par, ordem de escrita permutada → mesmo resultado (≠ τ da cisão, ≠ D_can)',
  }
}

/** K7 — comparação lexMax × rectCell no mesmo par (C0–L7 intacto). */
...
/** K7-extremos — lexMax × lexMin: duas orientações da mesma semilattice.
 *  Carrier discreto (marcas do relógio / índices de matriz), não ℝ. */
export function runK7LexExtremos (payloads = K_PAYLOADS) {
  const pares = [
    { label: 'x1,x2', a: payloads.x1, b: payloads.x2 },
    { label: 'x2,x1', a: payloads.x2, b: payloads.x1 },
    { label: 'x1p,x2', a: payloads.x1p, b: payloads.x2 },
  ...
  const comparacoes = pares.map(({ label, a, b }) => {
    const mx = valorReducao(PROBE_REDUCER.lexMax, a, b)
    const mn = valorReducao(PROBE_REDUCER.lexMin, a, b)
    return { label, a, b, lexMax: mx, lexMin: mn, iguais: mx === mn }
  })
  const mesmaLei = comparacoes.every((c) => c.iguais)
  const distintosOndeDesiguais = comparacoes.filter((c) => c.a !== c.b)
  return {
    protocolo: 'K7-lex-extremos',
    comparacoes,
    mesmaLei,
    leisDistintas: !mesmaLei,
    distintosEmParesDesiguais: distintosOndeDesiguais.every((c) => !c.iguais),
    leitura: mesmaLei ? 'mesma_realizacao' : 'duas_orientacoes_da_semilattice',
    leiC: null,
  }
}
 ...
 * L2–L4 são propriedades algébricas; L5=K6 (ordem de escrita; ≠ τ, ≠ D_can).
 */
...
/** Bateria L0–L7 sobre lexMax, lexMin, rectCell + análise de seleção. */
export function runLBateriaInvariantes (payloads = K_PAYLOADS) {
  ...
  return {
    ...
    congelado: {
      ...
      naoUnico: 'a condição não determina f — lexMax ≠ lexMin ≠ rectCell',
      extremos: 'L0–L7 não separam lexMax de lexMin — duas orientações da semilattice',
 ...
 *  Quatro marcas discretas; não ℝ. */
export const M_CADEIA = Object.freeze(['A0', 'B0', 'C0', 'D0'])

/** Relógio discreto do espaço das matrizes: índices 0..N−1, N=4.
 *  Mesma cardinalidade que E_ALFABETO; D(i)=(N−1)−i vive aqui, não em ℝ. */
export const M_RELOGIO = Object.freeze(['0', '1', '2', '3'])
...
export function runMBateriaOrdem (payloads = K_PAYLOADS, cadeia = M_CADEIA) {
  ...
  const minM1 = runM1MonoArg1(CANDIDATO.lexMin, cadeia)
  const minM2 = runM2MonoArg2(CANDIDATO.lexMin, cadeia)
  const rectM1 = runM1MonoArg1(CANDIDATO.rectCell, cadeia)
  const rectM2 = runM2MonoArg2(CANDIDATO.rectCell, cadeia)
  const relogio = {
    m0: runM0ControleOrdem(M_RELOGIO),
    lexMax: {
      M1: runM1MonoArg1(CANDIDATO.lexMax, M_RELOGIO),
      M2: runM2MonoArg2(CANDIDATO.lexMax, M_RELOGIO),
    },
    lexMin: {
      M1: runM1MonoArg1(CANDIDATO.lexMin, M_RELOGIO),
      M2: runM2MonoArg2(CANDIDATO.lexMin, M_RELOGIO),
    },
  }
  const kBat = runKBateriaCandidatos(payloads)
  const sintese = {
    lexMax: sinteseM(lexM1, lexM2),
    lexMin: sinteseM(minM1, minM2),
    rectCell: sinteseM(rectM1, rectM2),
  }
  const separa = sintese.lexMax.ambos !== sintese.rectCell.ambos
  const extremosIguais = sintese.lexMax.ambos === sintese.lexMin.ambos
  return {
    m0,
    lexMax: { M1: lexM1, M2: lexM2, sintese: sintese.lexMax },
    lexMin: { M1: minM1, M2: minM2, sintese: sintese.lexMin },
    rectCell: { M1: rectM1, M2: rectM2, sintese: sintese.rectCell },
    relogio,
    registro: {
      M1: { lexMax: lexM1.mono, lexMin: minM1.mono, rectCell: rectM1.mono },
      M2: { lexMax: lexM2.mono, lexMin: minM2.mono, rectCell: rectM2.mono },
      K: {
        lexMax: kBat.lexMax.sintese,
        lexMin: kBat.lexMin.sintese,
        rectCell: kBat.rectCell.sintese,
        K7leisDistintas: kBat.K7.leisDistintas,
        K7extremosDistintos: kBat.K7extremos.leisDistintas,
      },
    },
    separa,
    extremosIguais,
    pergunta: 'a ordem discreta tem autoridade sobre f ∈ A_min?',
    congelado: {
      autoridade: separa
        ? 'M1/M2 separam lexMax de rectCell — classificação, não escolha de C'
        : 'M1/M2 não separam os candidatos de A_min',
      extremos: extremosIguais
        ? 'M1/M2 não distinguem lexMax de lexMin — duas orientações da semilattice'
        : 'M1/M2 separam as duas orientações',
      naoUnico: 'a ordem discreta restringe; não determina uma lei única C',
      naoPromove: 'lexMax sobrevivente sob monotonia ≠ C',
      carrier: 'relógio = índices finitos da matriz; não ℝ',
    },
    referencia: 'thm:rn (ordem total discreta); A_min = L0–L7; K0–K7 caracterização',
    leiC: null,
  }
}
...
export function runEBateriaDobra (payloads = K_PAYLOADS, alfabeto = E_ALFABETO) {
  ...
  const minE = runEPartialCovariancia(CANDIDATO.lexMin, alfabeto)
  const rectE = runEPartialCovariancia(CANDIDATO.rectCell, alfabeto)
  const kBat = runKBateriaCandidatos(payloads)
  const sintese = {
    lexMax: sinteseE(lexE),
    lexMin: sinteseE(minE),
    rectCell: sinteseE(rectE),
  }
  const independentesDeIgualdade = !(sintese.lexMax.covariante && sintese.rectCell.covariante)
  const extremosIguais = sintese.lexMax.aplicavel === sintese.lexMin.aplicavel
    && sintese.lexMax.covariante === sintese.lexMin.covariante
    && sintese.lexMax.nIguais === sintese.lexMin.nIguais
  return {
    e0,
    lexMax: { E: lexE, sintese: sintese.lexMax },
    lexMin: { E: minE, sintese: sintese.lexMin },
    rectCell: { E: rectE, sintese: sintese.rectCell },
    registro: {
      E_partial: {
        lexMax: { aplicavel: lexE.aplicavel, covariante: lexE.covariante },
        lexMin: { aplicavel: minE.aplicavel, covariante: minE.covariante },
    ...
    congelado: {
      ...
      extremos: extremosIguais
        ? 'E_∂ não distingue lexMax de lexMin'
        : 'E_∂ separa as duas orientações',
      naoPromove: 'covariância sob a dobra ≠ C',
      carrier: 'alfabeto = marcas discretas do relógio da matriz; não ℝ',
    },
    referencia: 'fis:thm:troca-realizacao (D²=id no índice discreto); isolado da ordem',
    leiC: null,
  }
}
