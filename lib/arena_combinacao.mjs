...
export const PROBE_REDUCER = {
  ...
  /** analogia discreta fis:thm:central — célula (x1,x2) partida por ≤/< */
  rectCell: (x1, x2) => (x1 <= x2 ? 'dom:le' : 'dom:gt') + '|' + (x2 <= x1 ? 'im:le' : 'im:gt'),
}
...
export function runD3BateriaProbes (payloads = PROBE_DEFAULT) {
  const catalogo = [
    ...
    { nome: 'rectCell', reducer: PROBE_REDUCER.rectCell, R: '(x1,x2)↦célula central' },
  ]
...
export const CANDIDATO = {
  ...
  rectCell: { nome: 'rectCell', reducer: PROBE_REDUCER.rectCell, R: '(x1,x2)↦célula central' },
}
