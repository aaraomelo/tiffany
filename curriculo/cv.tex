...
\begin{itemize}
\item \textbf{Teoria} (57 pp.) --- uma lei única e as suas interpretações. \textbf{20 enunciados
e 21 provas}: 18 teoremas, 2 proposições, e \textbf{nenhum enunciado sem demonstração}. Uma única
definição em todo o volume --- a do corpo dual --- e tudo o resto derivado dela.
\item \textbf{Catálogo} (445 pp.) --- o bestiário dos corpos algébricos, cada um com a sua régua e
a sua dinâmica, e \textbf{239 medidores} citados nominalmente, em 322 chamadas.
...
0 falhas. \textbf{141 deles não usam um único número de vírgula flutuante}; a aritmética é inteira e
...
\smallskip
\noindent \emph{E ela não é uma invenção nossa}: a tabela de comutação de um inversor de três
níveis é o mesmo objeto --- cada fase toma $\{-1,0,+1\}$, e o \textbf{DTC multinível} decide pela
fase e pela amplitude do erro, \emph{sem modulador e sem histórico}. É a máquina sem memória em
silício, e é o meu trabalho de conclusão de 2018.
...
corre}: o \texttt{broca-so} (688 commits, $\sim$334\,000 linhas) orquestra agentes sobre um motor
...
O terceiro volume é um romance: \textbf{360 páginas, 26 partes, 145 capítulos, zero fórmulas}. Não
...
\lin{\emph{CristalChain} --- uma corrente cuja cifra \textbf{conserva}, e um contrato que se
liquida}{proposta aberta}

Uma corrente de blocos é, no fundo, uma pergunta sobre \textbf{conservação}: nada se pode criar do
nada, e nada se pode perder. Isso costuma ser garantido por convenção --- regras de emissão,
consenso, penalizações. \textbf{Aqui é uma propriedade da cifra}, e essa é a proposta.

\medskip
\lin{1. A cifra é o ouro branco, e a escolha é forçada}{}
A família metálica é $x^2 = nx + 1$, e nela o determinante é \textbf{sempre $-1$}: é a matriz de
Fibonacci, que \emph{anti}-conserva --- troca o sinal a cada passo. O \textbf{ouro branco} satisfaz
$x^2 = 4x - 1$, com o sinal trocado, e aí o determinante é $\mathbf{+1}$:

\begin{center}\small
\begin{tabular}{@{}llll@{}}
\toprule
 & equação & matriz & determinante \\
\midrule
ouro, prata, bronze & $x^2 = nx + 1$ & $[[n,1],[1,0]]$ & $-1$ \quad \emph{anti-conserva} \\
\textbf{ouro branco} & $x^2 = 4x - 1$ & $[[4,-1],[1,0]]$ & $\mathbf{+1}$ \quad \textbf{conserva} \\
\bottomrule
\end{tabular}
\end{center}

\noindent \textbf{Não é mais um metal na lista: está do outro lado do determinante}, e é por isso
que serve de cifra a um livro-razão. Uma corrente cifrada por $\det = -1$ não conserva o
invariante --- inverte-o. Medido em \texttt{tests/ouro\_branco.c}: a equação é exata em
$\mathbb{Z}[\sqrt3]$, as $12$ dobras saem dos ângulos e não de uma tabela, e o \textbf{controlo}
mostra que um $x^2 = nx - 1$ qualquer \emph{não} dá $12$ --- o $4$ não é um número escolhido a
gosto.

\medskip
\lin{2. O contrato não se assina: liquida-se --- e a paragem sai da álgebra}{}
A dificuldade conhecida de um contrato inteligente é \textbf{parar}. O estado da arte resolve-a por
fora, com um orçamento de execução (o \emph{gas}): o contrato corre até o crédito acabar. Aqui a
paragem é \textbf{interna}, porque toda órbita de um corpo finito fecha, e fecha \textbf{abaixo de
um teto que a própria régua dá}:

\begin{center}\small
\begin{tabular}{@{}rrr@{}}
\toprule
$q$ & passos até fechar & teto $q^2$ \\
\midrule
$3$ & $8$ & $9$ \\
$5$ & $20$ & $25$ \\
$11$ & $10$ & $121$ \\
...
\noindent \textbf{Nenhuma órbita passa o teto, e não há contador nenhum por fora.} E o número de
passos não é um número qualquer: é o \textbf{período de Pisano} $\pi(q)$ --- calculado por outro
caminho, a recorrência de Fibonacci $\bmod\ q$, sem tocar na órbita. \emph{São duas contas
independentes a dar o mesmo, e por isso podiam discordar.} E as outras duas
dificuldades canónicas caem no mesmo sítio: a \textbf{reentrância} não existe porque o agente que
executa é \emph{função do discriminante} --- a entrada não o escolhe, a régua escolhe-o; e o
\textbf{oráculo} deixa de ser preciso onde a metade que falta se \emph{deriva} da metade fornecida,
com resíduo $0$. Medido em \texttt{tests/smartcontract.c}, seis secções.

\smallskip
\noindent A diferença de fundo: \textbf{o contrato assinado} declara cláusulas e alguém verifica se
foram cumpridas; \textbf{o contrato que se liquida} recebe metade e deriva o resto, e liquidar é
simplesmente correr. Não há contraparte a honrar nem árbitro a julgar --- e a recusa também é uma
liquidação, não uma falha.

\medskip
\lin{O que existe e o que não existe --- para não haver equívoco}{}
\textbf{O que existe}: a álgebra, os medidores que a verificam, e o lastro descrito no corpo da obra
(a CristalChain é uma peça do mundo construído, com a sua função económica escrita). \textbf{O que
não existe}: rede, nós, consenso entre pares, ou qualquer coisa em produção. \textbf{Isto é uma
proposta de fundamento, não um produto} --- o que trago é a razão pela qual a cifra é aquela e não
outra, e procuro quem saiba da parte distribuída, que não é a minha.
